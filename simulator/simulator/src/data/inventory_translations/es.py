ES_PLOT = {
    ...
    'PROVINCE': 'Provincia',
    'STUDY_AREA': 'Zona_de_estudio',
    'MUNICIPALITY': 'Municipio',
    'FOREST': 'Monte',
    'PROV_REGION': 'Region_procedencia',
    ...
    'CONTINENTALITY': 'Continentalidad',  # (linear distance to the Mediterranean sea, km)
    'LONGITUDE': 'Longitud',
    'LATITUDE': 'Latitud',
    'ALTITUDE': 'Altitud',
    'AA_RAINFALL': 'Precipitacion_Media_Anual',  # (mm)
    'MA_TEMPERATURE': 'Temperatura_Media_Anual',  # (ºC)
    'SEPTEMBER_RAIN': 'Precipitacion_Septiembre',  # (mm)
    'SEPTEMBER_TEMP': 'Temperatura_Media_Septiembre',  # (ºC)
    'NOVEMBER_RAIN': 'Precipitacion_Noviembre',  # (mm)
    'NOVEMBER_TEMP': 'Temperatura_Media_Noviembre',  # (ºC)   
    ...
    'H_MIN': 'h_minima',  # (m)
    'MEAN_H': 'h_media',  # (m)
    'DOMINANT_H': 'Altura_dominante',  # (m)

    # Basic plot variables calculated - crown
    'CROWN_MEAN_D': 'd_medio_copa',  # (m)
    'CROWN_DOM_D': 'Do_copa',  # (m)
    'CANOPY_COVER': 'FCC',  # (%)
    'CANOPY_VOL': 'Volumen_copa',  # Canopy volume (m3)

    # Basic plot variables calculated - plot
    'SLENDERNESS_MEAN': 'Esbeltez',  # slenderness calculated by using mean values of height and dbh (cm/cm)
    'SLENDERNESS_DOM': 'Esbeltez_dominante',  # slenderness calculated by using top height and dbh values (cm/cm)
    ...
    'HART_STAGGERED': 'HartBecking__tresbolillo',  # Hart-Becking Index - S
    'SI': 'SI',  # (m)
    'REF_SI_AGE': 'SI_edad_referencia',  # SI reference age (years)
    'HEGYI_RADIUS': 'Radio_Hegyi',  # radius value to calculate the Hegyi competition index (m)

    # Plot variables calculated - volume
    'VOL': 'Volumen',  # (m3/ha)
    'BOLE_VOL': 'V_sin_corteza',  # (m3/ha)
    'BARK_VOL ': 'V_de_corteza',  # (m3/ha)
    'VOL_CUT_VOLUME': 'V_extraido',  # Volume harvested percentage (%)
    'DEAD_VOL': 'V_muerto',  # Volume of dead trees after an execution (m3/ha)
    ...
    'DEAD_WT': 'WT_muerta',  # WT of the dead trees after an execution (Tn/ha)
    'ING_WT': 'WT_incorporada',  # WT of the ingrowth trees after an execution (Tn/ha)

    # Plot variables calculated - wood uses
    'UNWINDING': 'V_desenrollo',  # unwinding = the useful wood volume unwinding destiny (m3/ha)
    'VENEER': 'V_chapa',  # veneer = the useful wood volume veneer destiny (m3/ha)
    'SAW_BIG': 'V_sierra_gruesa',  # saw_big = the useful wood volume big saw destiny (m3/ha)
    'SAW_SMALL': 'V_sierra',  # saw_small = the useful wood volume small saw destiny (m3/ha)
    'SAW_CANTER': 'V_sierra_canter',  # saw_canter = the useful wood volume canter saw destiny (m3/ha)
    ...
    'W_CORK': 'W_Corcho_Fresco',  # fresh cork biomass (Tn/ha)
    'TOTAL_W_DEBARK': 'W_descorchado',  # Total cork biomass (Tn/ha) extraxted from debark treatments
    'TOTAL_V_DEBARK': 'V_descorchado',  # Total cork volume (m3/ha) extracted from debark treatments

    # Pinus pinea special variables
    'ALL_CONES': 'Pinhas',  # total of cones of the plot (anual mean)
    'SOUND_CONES': 'Pinhas_sanas',  # total sound (healthy) cones of the plot (anual mean)
    'SOUND_SEEDS': 'Semillas_sanas',  # total sound (healthy) seeds of the plot (anual mean)
    ...
    'MARKETED_MUSH': 'Setas_comerciales',  # annual mushroom production of marketed species (mean value) (kg/ha)
    'MARKETED_LACTARIUS': 'Lactarius_comerciales',  # production of marketed Lactarius (kg/ha*year)
    'MUSHROOM_PRODUCTIVITY': 'ProducCion_setas',
    ...
    'CANOPY_VOL_SP1': 'Volumen_copa_sp1',  # Canopy volume (m3) for specie 1

    'CROWN_MEAN_D_SP2': 'd_medio_copa_sp2',  # (m)
    'CROWN_DOM_D_SP2': 'Do_copa_sp2',  # (m)
    'CANOPY_COVER_SP2': 'FCC_sp2',  # (%)        
    'CANOPY_VOL_SP2': 'Volumen_copa_sp2',  # Canopy volume (m3) for specie 2
    ...
    # Volume
    'VOL_SP1': 'V_con_corteza_sp1',  # (m3/ha)
    'BOLE_VOL_SP1': 'V_sin_corteza_sp1',  # (m3/ha)
    'BARK_VOL_SP1': 'V_corteza_sp1',  # (m3/ha) 

    'VOL_SP2': 'V_con_corteza_sp2',  # (m3/ha)
    'BOLE_VOL_SP2': 'V_sin_corteza_sp2',  # (m3/ha)
    'BARK_VOL_SP2': 'V_corteza_sp2',  # (m3/ha) 
    
    # Merchantable variables    
    'UNWINDING_SP1': 'V_desenrollo_sp1',  # unwinding = the useful wood volume unwinding destiny (m3/ha)
    'VENEER_SP1': 'V_chapa_sp1',  # veneer = the useful wood volume veneer destiny (m3/ha)
    'SAW_BIG_SP1': 'V_sierra_gruesa_sp1',  # saw_big = the useful wood volume big saw destiny (m3/ha)
    'SAW_SMALL_SP1': 'V_sierra_sp1',  # saw_small = the useful wood volume small saw destiny (m3/ha)
    'SAW_CANTER_SP1': 'V_sierra_canter_sp1',  # saw_canter = the useful wood volume canter saw destiny (m3/ha)
    ...
    'UNWINDING_SP2': 'V_desenrollo_sp2',  # unwinding = the useful wood volume unwinding destiny (m3/ha)
    'VENEER_SP2': 'V_chapa_sp2',  # veneer = the useful wood volume veneer destiny (m3/ha)
    'SAW_BIG_SP2': 'V_sierra_gruesa_sp2',  # saw_big = the useful wood volume big saw destiny (m3/ha)
    'SAW_SMALL_SP2': 'V_sierra_sp2',  # saw_small = the useful wood volume small saw destiny (m3/ha)
    'SAW_CANTER_SP2': 'V_sierra_canter_sp2',  # saw_canter = the useful wood volume canter saw destiny (m3/ha)
...
ES_TREE = {
    ...
    'quality': 'calidad',
    'shape': 'forma',
    'special_param': 'parametros_especiales',
    'remarks': 'observaciones',
    'age_130': 'edad_a_1.30',  # (years)
    'social_class': 'clase_sociologica',
    'tree_age': 'edad',  # (years)
    'coord_x': 'X',
    'coord_y': 'Y',
    'coord_z': 'Z',
    ...
    'stump_h': 'h_tocon',  # (m)
    'height': 'altura_total',  # total height (m)
    'height_i': 'height_i',  # increment in total tree height (m)
    'bark_1': 'corteza_1',  # bark thickness measurement 1 (mm)
    'bark_2': 'corteza_2',  # bark thickness measurement 2 (mm)
    'bark': 'corteza',  # mean bark thickness (mm)
    ...
    'slenderness': 'esbeltez',  # (cm/cm)
    'basal_area_i': 'basal_area_i',  # increment in basal area (cm2)
    'basal_area': 'area_basimetrica',  # (cm2)
    'bal': 'bal',  # (m2/ha)
    'ba_ha': 'g_ha',  # (m2/ha)
    ...
    'hcb': 'altura_bc',  # height crown base (m)
    'hlcw': 'altura_lcw',  # height of largest crown width (m)
    'cpa': 'cpa',  # crown projection area (m2)
    'crown_vol': 'vol_copa',  # crown volume (m3)

    # Volume variables
    'vol': 'v_con_corteza',  # volume with bark (dm3)
    'bole_vol': 'v_sin_corteza',  # volume without bark (dm3)
    'bark_vol': 'v_de_corteza',  # volume of bark (dm3)
    ...
    'w_cork': 'w_corcho_fresco',  # fresh cork biomass (kg)
    ...
    # Wood uses variables    
    'unwinding': 'v_desenrollo',  # unwinding = the useful wood volume unwinding destiny (dm3)
    'veneer': 'v_chapa',  #v eneer = the useful wood volume veneer destiny (dm3)
    'saw_big': 'v_sierra_gruesa',  # saw_big = the useful wood volume big saw destiny (dm3)
    'saw_small': 'v_sierra',  # saw_small = the useful wood volume small saw destiny (dm3)
    'saw_canter': 'v_sierra_canter',  # saw_canter = the useful wood volume canter saw destiny (dm3)
    ...
    # Quercus suber special variables
    'dbh_oc': 'dbh_con_corcho',  # dbh over cork (cm) - Quercus suber
    ...
    'cork_cycle': 'ciclo_de_corcho',  # moment to obtain cork data; 0 to the moment just immediately before the stripping process,
    'count_debark': 'n_descorches',  # Accountant of debark operations realised over each tree
    'total_w_debark': 'w_descorchado',  # Cork biomass accumulator (kg) from the extracted cork of each tree with debark operations
    'total_v_debark': 'v_descorchado',  # Cork volume accumulator (dm3) from the extracted cork of each tree with debark operations
...
# or 1 to the moment after the stripping process or at an intermediate age of the cork cycle production - Quercus suber
    ...
    'sound_seeds': 'semillas_sanas',  # total sound seeds of the tree (anual mean)
