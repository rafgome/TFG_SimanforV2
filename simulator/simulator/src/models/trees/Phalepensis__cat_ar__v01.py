...
# Pure stands of Pinus halepensis located on CAtaluña and Aragón (Spain)
# Version 01
#
...
class PinusHalepensisCataluña(TreeModel):
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Phalepensis__cat_ar__v01'  # set the model name to show it at the output
            ...
            Model.aplication_area = 'Aragón & Cataluña (Spain)'  # area recommended to use the model; just write 'none' if it is not defined yet
            Model.valid_prov_reg = '1, 2, 3, 4, 5, 6'  # provenance regions recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/arbol/Phalepensis_cat_ar_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/arbol/Phalepensis_cat_ar_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                  Running: Pinus halepensis model (Cataluña). Plot:', plot.plot_id                   )
        print('#--------------------------------------------------------------------------------------------------#')
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if old_tree.specie == Model.specie_ifn_id:  # specie condition
                ...
                ht = 1.3 + (plot.mean_h - 1.3)*math.exp(0.3532*(1 - plot.qm_dbh/new_tree.dbh))*math.exp(0.0315*(new_tree.dbh/plot.qm_dbh - 1/new_tree.dbh))
