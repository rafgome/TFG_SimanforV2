...
class PinusRadiataGalicia(TreeModel):
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Pradiata__gal__v01'  # set the model name to show it at the output
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/arbol/Pradiata_gal_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/arbol/Pradiata_gal_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                    Running: Pinus radiata model (Galicia). Plot:', plot.plot_id                     )
        print('#--------------------------------------------------------------------------------------------------#')
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if tree.specie == Model.specie_ifn_id:  # specie condition
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        a = 0.0296
                        b = 1.633
                        tree.add_value('height', (1.3**b + (plot.dominant_h**b - 1.3**b) * ((1 - math.exp(-a*tree.dbh)) / (1 - math.exp(-a*plot.dominant_dbh))))**(1/b))
                    ...
                    self.crown(tree, plot, 'initialize', plot.age)  # activate crown variables
        ...
        try:  # errors inside that construction will be announced
            ...
            for tree in plot_trees:  # for each tree...
                ...
                #-----------------------------------BASAL_AREA-----------------------------------------#
                    ...
                    self.crown(tree, plot, 'update_model', new_age)  # activate crown variables
    ...
    def crown(self, tree: Tree, plot: Plot, func, new_age):
        """
        ...
        try:  # errors inside that construction will be announced
            ...
            tree.add_value('lcw', 0.06185*(tree.dbh**1.185)*math.exp(-0.009319*plot.basal_area -0.009502*new_age))  # maximum crown-width (m)
