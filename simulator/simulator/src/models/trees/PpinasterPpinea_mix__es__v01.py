...
# Mixed stands of Pinus pinaster and Pinus pinea located on Spain
# Version 01
#
...
class PpinasterPpineaMix(TreeModel):

    def __init__(self, configuration=None):
        super().__init__(name="PpinasterPpineaMix", version=1)
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('            Running: Pinus pinaster - Pinus pinea mixed model (Spain). Plot:', plot.plot_id        )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        beta0 =  0.2315 * plot.qm_dbh_sp1 + 0.3722 * plot.sp1_proportion
                        h_sp1 = 1.3 + (beta0 * (1/tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_height - 1.3)) ** (1/2)) ** (-2)
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        beta0 = 4.0383 - 0.022 * tree.bal + 1.5266 * plot.sp2_proportion - 0.1198 * M
                        h_sp2 = 1.3 + (beta0 * (1/tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_height - 1.3)) ** (1/2)) ** (-2)
            ...
            SDImax_2 = math.exp(13.304 + ((-2.2518 + 0.0077 * M) * math.log(25)))
            ...
            if plot.qm_dbh_sp2 != 0:
                SDI_2 = plot.density_sp2 * ((25/plot.qm_dbh_sp2) ** (-2.2518 + 0.0077 * M))
            else:
                SDI_2 = 0
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(old_tree.specie) == int(plot.id_sp1):  # specie 1 condition
                ...
                bai5_sp1 = math.exp(-1.2902 - 0.044 * old_tree.dbh + 2.3163 * math.log(old_tree.dbh) - 0.0148 * plot.qm_dbh - 0.0154 * old_tree.basal_area)
                ...
                beta0 = 0.2315 * plot.qm_dbh_sp1 + 0.3722 * plot.sp1_proportion
                h_sp1 = 1.3 + (beta0 * (1/new_tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_h - 1.3)) ** (1/2)) ** (-2)
                ...
                bai5_sp2 = math.exp(1.8982 + 1.1735*math.log(old_tree.dbh) - 0.0185 * plot.qm_dbh - 0.0329 * old_tree.basal_area_intrasp - 0.0261 * M)
                new_tree.sum_value("basal_area", bai5_sp2)  # update basal area (cm2) 

                new_tree.add_value('dbh', 2*math.sqrt(new_tree.basal_area/math.pi))  # upload dbh (cm)

                beta0 = 4.0383 - 0.022 * new_tree.bal + 1.5266 * plot.sp2_proportion - 0.1198 * M
                h_sp2 = 1.3 + (beta0 * (1/new_tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_h - 1.3)) ** (1/2)) ** (-2)
    ...
    def merchantable(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            usage, merch_list = TreeModel.merch_calculation_mix(tree, plot, class_conditions, PpinasterPpineaMix)
    ...
    def biomass(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            elif int(tree.specie) == int(plot.id_sp2):  # specie 2 condition [Pinus pinea]
