...
# Pure stands of Quercus pyrenaica located on Castilla y León (Spain)
# Version 01
#
...
class QuercusPyrenaicaCyL(TreeModel):
    ...
    global dead_trees_list  # that variable is needed to record dead trees and calculate CESEFOR deadwood indexes
    dead_trees_list = []
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Qpyrenaica__cyl__v01'  # set the model name to show it at the output
            Model.specie_ifn_id = 43  # Set the model specie ID to mark the trees of different species         
            Model.exec_time = 10  # recommended executions time to use that model
            Model.aplication_area = 'Castilla y León (Spain)'  # area recommended to use the model; just write 'none' if it is not defined yet
            Model.valid_prov_reg = '3, 4, 5, 6, 7, 8, 9, 11, 12'  # provenance regions recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/arbol/Qpyrenaica_cyl_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/arbol/Qpyrenaica_cyl_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        """
         ...
         """
        # TODO: en este modelo se calculan: carbon, wbl0_7, diversity indexes, growth variables; deben incluirse en TODOS los modelos
        print('#------------------------------------------------------------------------------------------------- #')
        print('                Running: Quercus pyrenaica model (Castilla y León). Plot:', plot.plot_id             )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            if plot.si == 0 or plot.si == '':
                ...
                if H0 != 0:
                    ...
                    SI = math.exp(X) * pow(t2, parA1 + parA2 * X)
                else:
                    SI = 0                    
                plot.add_value('SI', SI)  # Site Index (m) calculation to 60 years as reference
            ...
            # TODO: nuevo, incluir en los demás modelos
            TreeEquations.null_growth(plot_trees)  # set growth variables as 0
            ...
            SDImax_1 = math.exp((9.809 + 0.677 * math.log(M)) + ((-1.5369 - 0.0047 * M) * math.log(25)))
            plot.add_value('REINEKE_MAX_SP1', SDImax_1)

            # Calculate both SDI values
            if plot.qm_dbh != 0:
                SDI_1 = plot.density * ((25 / plot.qm_dbh) ** (-1.5369 - 0.0047 * M))
            else:
                SDI_1 = 0
            ...
            #TreeEquations.get_shannon_and_pielou_index(plot, plot_trees)

            # set deadwood index (CESEFOR) as - (no dead trees at initialize)
            plot.add_value('DEADWOOD_INDEX_CESEFOR_G', '-')
            plot.add_value('DEADWOOD_INDEX_CESEFOR_V', '-')
    ...
    def survival(self, time: int, plot: Plot, tree: Tree):
        ...
        # I decided to apply survival and ingrowth in a gradual way, preserving the original values for the 0.90*SDImax
        # on survival and 0.75*SDImax on ingrowth

        if tree.specie == Model.specie_ifn_id:  # specie condition

            # add the values needed for CESEFOR deadwood index to the list
            dead_expan = 0.1118
            global dead_trees_list

            if plot.reineke_sp1 > (0.90 * plot.reineke_max_sp1):  # SDI condition

                dead_expan = dead_expan * 1
                dead_trees_list.append([tree.tree_id, tree.dbh, tree.expan, tree.basal_area, tree.vol, dead_expan])

                return (1 - dead_expan)  # average observed mortality rate of http://dx.doi.org/10.1051/forest/2010046

            elif plot.reineke_sp1 > (0.85 * plot.reineke_max_sp1):  # SDI condition

                dead_expan = dead_expan * 0.75
                dead_trees_list.append([tree.tree_id, tree.dbh, tree.expan, tree.basal_area, tree.vol, dead_expan])

                return (1 - dead_expan)

            elif plot.reineke_sp1 > (0.80 * plot.reineke_max_sp1):  # SDI condition

                dead_expan = dead_expan * 0.50
                dead_trees_list.append([tree.tree_id, tree.dbh, tree.expan, tree.basal_area, tree.vol, dead_expan])

                return (1 - dead_expan)

            elif plot.reineke_sp1 > (0.75 * plot.reineke_max_sp1):  # SDI condition

                dead_expan = dead_expan * 0.25
                dead_trees_list.append([tree.tree_id, tree.dbh, tree.expan, tree.basal_area, tree.vol, dead_expan])
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            # TODO: hay que crear 3 variables para el crecimiento: dbhi, gi, hi
            if old_tree.specie == Model.specie_ifn_id:  # specie condition

                new_tree.sum_value('tree_age', time)

                # growth old variables  # TODO: nuevo
                dbh_old = old_tree.dbh
                h_old = old_tree.height
                g_old = old_tree.basal_area
                ...
                # growth variables  # TODO: nuevo
                new_tree.add_value('dbh_i', new_tree.dbh - dbh_old)  # cm
                new_tree.add_value('height_i', new_tree.height - h_old)  # h
                new_tree.add_value('basal_area_i', new_tree.basal_area - g_old)  # cm2
    ...
    def ingrowth(self, time: int, plot: Plot):
        ...
        if plot.reineke_sp1 < (0.75 * plot.reineke_max_sp1):  # SDI condition

            gradual = 1
            return (357.9 / 1315.2) * plot.basal_area * gradual
        ...
        elif plot.reineke_sp1 < (0.90 * plot.reineke_max_sp1):  # SDI condition

            gradual = 0.25
            return (357.9 / 1315.2) * plot.basal_area * gradual
            # m2/ha; average observed ingrowth on fitting dataset on http://dx.doi.org/10.1051/forest/2010046

        else:

            return 0

        # prob_ingrowth: float = 1/(1 + math.exp( - (4.4765 - 0.1167*plot.qm_dbh - 0.3492*plot.mean_h)))

        # if prob_ingrowth >= 0.58:
        #     N_added = math.exp(8.60716 - 0.64247*math.log(plot.density) + 197.99212/(plot.mean_dbh**2))
        #     if N_added < 0:
        #         return 0.0
        #     else:
        #         ba_added = math.pow(0.1/2, 2)*math.pi*N_added
    ...
    def ingrowth_distribution(self, time: int, plot: Plot, area: float):
        ...
        # first diametric class (empty)
        cd_0 = [0, 7.5, 0*area]
        distribution.append(cd_0)

        # second diametric class
        cd_1 = [7.5, 12.5, 1*area]
        distribution.append(cd_1)

        # third diametric class
        cd_2 = [12.5, 100, 0*area]
    ...
    def update_model(self, time: int, plot: Plot, trees: list):
        ...
        try:  # errors inside that construction will be announced
            ...
            # TODO: nuevo
            # before start, I will update the plot information with the new trees created due to ingrowth

            for tree in plot_trees:  # for each tree...

                if tree.id > 1000000:  # ingrowth condition
            ...
            # get deadwood index (CESEFOR)
            plot.add_value('DEADWOOD_INDEX_CESEFOR_G', TreeEquations.get_deadwood_index_cesefor_g(plot, dead_trees_list))
            plot.add_value('DEADWOOD_INDEX_CESEFOR_V', TreeEquations.get_deadwood_index_cesefor_v(plot, dead_trees_list))

            # delete content of global variable to the next simulation
            dead_trees_list = []
    ...
    def merchantable(self, tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            class_conditions = [['saw_big', 2.5 / ht, 40, 200], ['saw_small', 2.5 / ht, 25, 200],
                                ['saw_canter', 1.05 / ht, 15, 28], ['chips', 1 / ht, 5, 1000000],
                                ['saw_big_liferebollo', 1.5 / ht, 30, 200], ['saw_small_liferebollo', 2 / ht, 25, 200],
                                ['staves_intona', 1 / ht, 40, 200], ['bottom_staves_intona', 0.6 / ht, 40, 200],
                                ['wood_panels_gamiz', 2.5 / ht, 30, 200], ['mix_garcia_varona', 2.5 / ht, 30, 200]] 
    ...
    def merchantable_plot(self, plot: Plot, plot_trees):
        ...
        try:  # errors inside that construction will be announced
            ...
            plot_saw_big_liferebollo = plot_saw_small_liferebollo = plot_staves_intona = plot_bottom_staves_intona = plot_wood_panels_gamiz = plot_mix_garcia_varona = 0
            ...
            for tree in plot_trees:  # for each tree, we are going to add the simple values to the plot value
                ...
                if tree.specie == Model.specie_ifn_id:  # specie condition
                    ...
                    plot_saw_big_liferebollo += tree.saw_big_liferebollo*tree.expan
                    plot_saw_small_liferebollo += tree.saw_small_liferebollo*tree.expan
                    plot_staves_intona += tree.staves_intona*tree.expan
                    plot_bottom_staves_intona += tree.bottom_staves_intona*tree.expan
                    plot_wood_panels_gamiz += tree.wood_panels_gamiz*tree.expan
                    plot_mix_garcia_varona += tree.mix_garcia_varona*tree.expan
            ...
            plot.add_value('SAW_BIG_LIFEREBOLLO', plot_saw_big_liferebollo/1000)
            plot.add_value('SAW_SMALL_LIFEREBOLLO', plot_saw_small_liferebollo/1000)
            plot.add_value('STAVES_INTONA', plot_staves_intona/1000)
            plot.add_value('BOTTOM_STAVES_INTONA', plot_bottom_staves_intona/1000)
            plot.add_value('WOOD_PANELS_GAMIZ', plot_wood_panels_gamiz/1000)
            plot.add_value('MIX_GARCIA_VARONA', plot_mix_garcia_varona/1000)
    ...
    def biomass(self, tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            tree.add_value('carbon_heartwood', carbon_heartwood)  # (kg)
            tree.add_value('carbon_sapwood', carbon_sapwood)  # (kg)
            tree.add_value('carbon_bark', carbon_bark)  # (kg)
            tree.add_value('carbon', carbon)  # carbon = total carbon biomass (kg)
    ...
    def biomass_plot(self, plot: Plot, plot_trees):
        ...
        try:  # errors inside that construction will be announced
            ...
            plot_wsw = plot_wsb = plot_w_cork = plot_wthickb = plot_wstb = plot_wb2_7 = plot_wb2_t = plot_wthinb = \
                plot_wb05 = plot_wl = plot_wtbl = plot_wbl0_7 = plot_wr = plot_wt = plot_carbon_heatwood = \
                plot_carbon_sapwood = plot_carbon_bark = plot_carbon = 0
            ...
            for tree in plot_trees:  # for each tree, we are going to add the simple values to the plot value
                ...
                if tree.specie == Model.specie_ifn_id:  # specie condition
                    ...
                    #plot_carbon += plot_wt*(0.1841*0.4578 + 0.5427*0.4558 + 0.2732*0.4582)  # bark, sapwood and heartwood proportions

            plot_carbon_heatwood = plot_wt * (0.2732 * 0.4582)
            plot_carbon_sapwood = plot_wt * (0.5427 * 0.4558)
            plot_carbon_bark = plot_wt * (0.1841 * 0.4578)
            plot_carbon = plot_wt*(0.1841*0.4578 + 0.5427*0.4558 + 0.2732*0.4582)
            ...
            plot.add_value('CARBON_HEARTWOOD', plot_carbon_heatwood/1000)  # (Tn/ha)
            plot.add_value('CARBON_SAPWOOD', plot_carbon_sapwood/1000)  # (Tn/ha)
            plot.add_value('CARBON_BARK', plot_carbon_bark/1000)  # (Tn/ha)
            plot.add_value('CARBON', plot_carbon/1000)  # carbon = total carbon biomass (Tn/ha)
