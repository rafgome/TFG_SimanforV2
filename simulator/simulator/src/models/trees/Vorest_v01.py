...
#
# Single tree growing model dependent from distance, developed to
# Pure stands of Abies pinsapo located on Sierra del Pinar de Grazalema (Spain)
# Version 01
#
...
class Vorest(TreeModel):

    def __init__(self, configuration=None):
        super().__init__(name="Vorest", version=1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.specie_ifn_id = 32  # Set the model specie ID to mark the trees of different species         
    ...
    def spatial_calculations(self, plot_trees, plot):
        """
        """

        try:  # errors inside that construction will be announced

            p = list()  # tree list that will contain [[x,y],w] information for each tree, to calculate Woronoi
            c = 0.8  # scaling parameter - w_voronoi calculation
            g = 0.2  # scaling parameter - w_voronoi calculation

            a = 0.0920  # PGS calculation parameter
            b = 0.5380  # PGS calculation parameter

            for tree in plot_trees:  # for each tree...
                if tree.specie == Model.specie_ifn_id:  # specie condition

                    tree.add_value('w_voronoi', tree.dbh*g/plot.mean_dbh)  # woronoi weight calculated
                    p.append([[tree.coord_x, tree.coord_y], tree.w_voronoi])  # tree information added to the list


            W = TreeModel.voronoi(p)  # Woronoi poligons calculation
            neighbours = TreeModel.neighbours(W[0], p)  # neighbours matrix calculation
            # ogs list will contain: [[x,y], w, ogs], being x and y coordinates; w the weight of the tree (Voronoi),
            # and ogs the OGS value of the tree
            ogs = TreeModel.calculateRadius(p)

            general_counter = 0  # counter used on for loop
            ...
            for tree in plot_trees:  # for each tree...
                ...
                for i in neighbours[general_counter]:  # for each neighbour tree...
                    #print(neighbours[general_counter])

                    if i == 1:  # if a tree on the list is a neighbour..
                        mean_dbh_list.append(plot_trees[specific_counter])  # the tree is added to a list
                        #print(plot_trees[specific_counter].dbh)
                        specific_counter += 1


                tree.add_value('neighbours_mean_dbh', TreeModel.mean_dbh(mean_dbh_list))  # mean dbh value of neighbour trees is calculated
                if tree.neighbours_mean_dbh != 0:
                    tree.add_value('w_voronoi', tree.dbh*(c/tree.neighbours_mean_dbh - g/plot.mean_dbh))  # woronoi weight REcalculated

                cw = math.exp(a + b*math.log(tree.dbh))  # crown width (m) 
                tree.add_value('lcw', cw)

                pgs = (math.pi/4)*(cw**2)  # potential growing space of tree i in year t estimated as the crown projection area of an open grown tree of the same dbh
                tree.add_value('pgs', pgs)

                tree.add_value('ogs', ogs[general_counter][2])

                tree.add_value('rel_area', tree.ogs/tree.pgs)
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                        Running: Vorest model (xx). Plot:', plot.plot_id                     )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            ##### Calculations related to Voronoi #####

            plot.add_value('MEAN_DBH', TreeModel.mean_dbh(plot_trees))  # calculate plot mean dbh value

            self.spatial_calculations(plot_trees, plot)

            plot.add_value('DEAD_DENSITY', '')  # change the value from '' to 0 in orde to print that information at the summary sheet
            plot.add_value('ING_DENSITY', '')  # change the value from '' to 0 in orde to print that information at the summary sheet
    ...
    def survival(self, time: int, plot: Plot, tree: Tree):
        ...
        Source:
        Doc.: Abellanas B, Abellanas M, Pommerening A, Lodares D & Tavira SC (2016). A forest simulation approach using weighted Voronoi diagrams. An application to Mediterranean fir Abies pinsapo Boiss stands. Forest systems, 25(2), 6
        Ref.: Abellanas et al, 2016
        """

        if tree.specie == Model.specie_ifn_id:  # specie condition

            #---> se puede buscar la posibilidad de hacer rangos de mortalidad, p.e. 10/20/30% mortalidad

            #pd5 = incremento relativo dbh en 5 años
            #id = dbh tras ejecución
            #dbh = dbh antes de la ejecución
            #alpha = 0.18
            #beta = 0.80

            #pd5 = (id - dbh)/dbh5

            #if pd5 <= (alpha/(id**beta)):
            #    return 0
            #else:
            #    return 1
            
            return 1  # borrar esta línea
        else:
            return 1
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if old_tree.specie == Model.specie_ifn_id:  # specie condition
                ...
                #id_pot = potential diameter increment of tree i at time t            
                A = 69.2108
                p = 2.492
                k = 0.022

                id_pot = A*((1 - math.exp(-k*old_tree.dbh))**(p - 1))*k*p*math.exp(-k*old_tree.dbh)


                # competition

                ### ver como programarlo

                #d1 = arithmetic mean of the stem diameters of the nearest neighbors
                #d2 = plot.mean_dbh  # arithmetic mean stand diameter
                #c = 0.8
                #g = 0.2
                ...
                #w = dbh*g/d2
                #print('w', w)
                #new_tree.add_value('voronoi_w', w)

                # cálculo definitivo

                v = 0.6522

                dbhi5 = id_pot*(1 - v*(1 - old_tree.rel_area))  # diameter increment of tree i at time step t

                #w = dbh*(c/d1 + g/d2)



                new_tree.sum_value("dbh", dbhi5)

                self.mortality(dbhi5, (new_tree.dbh - dbhi5), new_tree.dbh)
    ...
    def mortality(self, dbhi5, dbh_0, dbh_5):
        """
        Source:
        Doc.: Abellanas B, Abellanas M, Pommerening A, Lodares D & Tavira SC (2016). A forest simulation approach using weighted Voronoi diagrams. An application to Mediterranean fir Abies pinsapo Boiss stands. Forest systems, 25(2), 6
        Ref.: Abellanas et al, 2016
        ...
        #pd5 = incremento relativo dbh en 5 años
        #dbh_5 = dbh tras ejecución
        #dbh_0 = dbh antes de la ejecución
        #dbhi5 = incremento dbh en la ejecución
        alpha = 0.18
        beta = 0.80

        pd5 = (dbh_5 - dbh_0)/dbhi5

        if pd5 <= (alpha/(dbh_5**beta)):
            return 0
        else:
            return 1
    ...
    def merchantable(self, tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            usage, merch_list = TreeModel.merch_calculation(tree, class_conditions, Vorest)
