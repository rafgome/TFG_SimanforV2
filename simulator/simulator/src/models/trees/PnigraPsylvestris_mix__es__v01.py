...
# Mixed stands of Pinus nigra and Pinus sylvestris located on Spain
# Version 01
#
...
class PnigraPsylvestrisMix(TreeModel):
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('            Running: Pinus nigra - Pinus sylvestris mixed model (Spain). Plot:', plot.plot_id        )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition
                    ...
                    #    beta0 = 1.4454 - 0.0136*tree.bal + 0.04743*plot.qm_dbh
                    ...
                    #if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                    #    beta1 = 0.8792 - 0.0068*tree.bal + 0.0465*plot.qm_dbh
                    #    h_sp2 = 1.3 + (beta1*(1/tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_height - 1.3))**(1/3))**(-3)
            ...
            SDImax_1 = math.exp((11.963 + 0.268*math.log(M)) - 1.870*math.log(25))
            ...
            if plot.qm_dbh_sp1 != 0:
                SDI_1 = plot.density_sp1*((25/plot.qm_dbh_sp1)**(-1.87))
            else:
                SDI_1 = 0
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(old_tree.specie) == int(plot.id_sp1):  # specie 1 condition
                ...
                bai5_sp1 = math.exp(-1.7176 - 0.0495*old_tree.dbh + 2.3873*math.log(old_tree.dbh) + 0.0187*old_tree.height - 0.0464*plot.qm_dbh - 0.0255*old_tree.basal_area_intrasp - 0.0095*old_tree.basal_area_intersp - 0.0087*old_tree.bal_intersp + 0.0158*M)
                ...
                beta0 = 1.4454 - 0.0136*new_tree.bal + 0.04743*plot.qm_dbh
                ...
                bai5_sp2 = math.exp(-1.3287 - 0.044*old_tree.dbh + 2.2713*math.log(old_tree.dbh) - 0.0382*plot.qm_dbh - 0.0181*old_tree.basal_area_intrasp - 0.0218*old_tree.basal_area_intersp - 0.0018*old_tree.bal_intrasp - 0.008*old_tree.bal_intersp + 0.0081*M)
                new_tree.sum_value("basal_area", bai5_sp2)  # update basal area (cm2) 

                new_tree.add_value('dbh', 2*math.sqrt(new_tree.basal_area/math.pi))  # upload dbh (cm)

                beta1 = 0.8792 - 0.0068*new_tree.bal + 0.0465*plot.qm_dbh
                h_sp2 = 1.3 + (beta1*(1/new_tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_h - 1.3))**(1/3))**(-3)
    ...
    def merchantable(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition
                ...
                class_conditions = [['saw_big', 2.5/ht, 40, 200], ['saw_small', 2.5/ht, 25, 200], ['saw_canter', 2.5/ht, 15, 28],
                                 ['post', 6/ht, 15, 28], ['stake', 1.8/ht, 6, 16], ['chips', 1/ht, 5, 1000000]]
