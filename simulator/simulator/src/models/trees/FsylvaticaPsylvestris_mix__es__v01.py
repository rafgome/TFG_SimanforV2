...
# Mixed stands of Fagus sylvatica and Pinus sylvestris located on Spain
# Version 01
#
...
class FsylvaticaPsylvestrisMix(TreeModel):

    def __init__(self, configuration=None):
        super().__init__(name="FsylvaticaPsylvestrisMix", version=1)
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('            Running: Fagus sylvatica - Pinus sylvestris mixed model (Spain). Plot:', plot.plot_id        )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        beta0 = -0.0049 * tree.bal + 0.02429 * plot.qm_dbh_sp1 + 0.6819 * plot.sp1_proportion
                        h_sp1 = 1.3 + (beta0 * (1/tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_height - 1.3)) ** (1/3)) ** (-3)
            ...
            SDImax_1 = math.exp(13.279 + ((-2.1438 + 0.0031 * M) * math.log(25)))
            ...
            if plot.qm_dbh_sp1 != 0:
                SDI_1 = plot.density_sp1 * ((25/plot.qm_dbh_sp1) ** (-2.1438 + 0.0031 * M))
            else:
                SDI_1 = 0
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(old_tree.specie) == int(plot.id_sp1):  # specie 1 condition
                ...
                bai5_sp1 = math.exp(-3.0448 - 0.0378 * old_tree.dbh + 2.3552 * math.log(old_tree.dbh) + 0.0421 * old_tree.height - 0.0159 * plot.qm_dbh - 0.0361 * old_tree.basal_area_intrasp - 0.0214 * old_tree.basal_area_intersp + 0.0178 * M)
                ...
                beta0 = -0.0049 * new_tree.bal + 0.02429 * plot.qm_dbh_sp1 + 0.6819 * plot.sp1_proportion
                ...
                bai5_sp2 = math.exp(-1.9057 - 0.0366 * old_tree.dbh + 2.2551 * math.log(old_tree.dbh) + 0.0173 * old_tree.height - 0.0224 * plot.qm_dbh - 0.0162 * old_tree.basal_area_intrasp - 0.0305 * old_tree.basal_area_intersp + 0.0083 * M)
    ...
    def merchantable(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            usage, merch_list = TreeModel.merch_calculation_mix(tree, plot, class_conditions, FsylvaticaPsylvestrisMix)
    ...
    def biomass(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition [Fagus sylvatica]
