...
# Mixed stands of Pinus sylvestris and Quercus pyrenaica located on Spain
# Version 01
#
...
class PsylvestrisQpyrenaicaMix(TreeModel):

    def __init__(self, configuration=None):
        super().__init__(name="PsylvestrisQpyrenaicaMix", version=1)
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('            Running: Pinus sylvestris - Quercus pyrenaica mixed model (Spain). Plot:', plot.plot_id        )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition
                    ...
                    #    beta0 = 0.0038*M
                    #    h_sp1 = 1.3 + (plot.dominant_h - 1.3)*math.exp(beta0*(1 - plot.dominant_dbh/new_tree.dbh) + beta1*(1/plot.dominant_dbh - 1/new_tree.dbh))
                    ...
                    #if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                    #    beta2 = - 0.0091*new_tree.bal + 0.0163*plot.qm_dbh_sp2 + 1.2693*plot.sp2_proportion
                    #    h_sp2 = 1.3 + (beta2*(1/new_tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_h - 1.3))**(0.5))**(-2)
            ...
            SDImax_2 = math.exp((9.809 + 0.677*math.log(M)) + ((-1.5369 - 0.0047*M)*math.log(25)))
            ...
            if plot.qm_dbh_sp2 != 0:
                SDI_2 = plot.density_sp2*((25/plot.qm_dbh_sp2)**(-1.5369 - 0.0047*M))
            else:
                SDI_2 = 0
            ...
            aaa = (plot.reineke_sp1 + (plot.reineke_sp2 * (plot.reineke_max_sp1/plot.reineke_max_sp2)))
            if aaa != 0:
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(old_tree.specie) == int(plot.id_sp1):  # specie 1 condition
                ...
                bai5_sp1 = math.exp(-1.1121 - 0.0374*old_tree.dbh + 2.2161*math.log(old_tree.dbh) + 0.0191*old_tree.height - 0.0447*plot.qm_dbh - 0.0206*old_tree.basal_area_intrasp - 0.0024*old_tree.bal_intrasp + 0.0133*M)
                ...
                bai5_sp2 = math.exp(-2.2915 - 0.0568*old_tree.dbh + 2.2302*math.log(old_tree.dbh) + 0.0671*old_tree.height - 0.0258*old_tree.basal_area_intrasp - 0.0174*old_tree.basal_area_intersp - 0.0132*old_tree.bal_intersp)
    ...
    def merchantable(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            usage, merch_list = TreeModel.merch_calculation_mix(tree, plot, class_conditions, PsylvestrisQpyrenaicaMix)
