...
class PinusSylvestrisSIM(TreeModel):
    ...
    def initialize(self, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            plot.add_value('REINEKE_VALUE', -1.7462 + 0.0007 * plot.martonne)  # r constant value of SDI to the species of the model (-1.605 as default)  
            
            if plot.martonne != 0 and plot.martonne != '':  # if we have Martonne index calculated, we calculate also SDImax
                SDImax = math.exp(12.579 + ((-1.7462 + 0.0007 * plot.martonne) * math.log(25)))
