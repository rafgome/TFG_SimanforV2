...
# Mixed stands of Pinus halepensis and Pinus pinaster located on Spain
# Version 01
#
...
class PhalepensisPpinasterMix(TreeModel):

    def __init__(self, configuration=None):
        super().__init__(name="PhalepensisPpinasterMix", version=1)
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('            Running: Pinus halepensis - Pinus pinaster mixed model (Spain). Plot:', plot.plot_id        )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            # Specie 1 = Pinus halepensis (24)
            # Specie 2 = Pinus pinaster (26)
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        beta0 = -0.0172 * tree.bal + 0.0734 * plot.qm_dbh + 1.5234 * plot.sp1_proportion
                        h_sp1 = 1.3 + (beta0 * (1/tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_height - 1.3)) ** 0.5) ** (-2)
                    ...
                    # no SI available, needed on Phal crown
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        beta0 = 1.2129 - 0.0402 * tree.bal + 0.0431 * plot.qm_dbh_sp2
                        h_sp2 = 1.3 + (beta0 * (1/tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_height - 1.3)) ** 0.5) ** (-2)
            ...
            SDImax_1 = math.exp((11.641 + 0.489 * math.log(M)) + ((-1.8491 - 0.0035 * M) * math.log(25)))
            plot.add_value('REINEKE_MAX_SP1', SDImax_1)
            SDImax_2 = math.exp((9.241 + 0.886 * math.log(M)) + ((-1.5559 - 0.0095 * M) * math.log(25)))
            plot.add_value('REINEKE_MAX_SP2', SDImax_2)


            # Calculate both SDI values
            if plot.qm_dbh_sp1 != 0:
                SDI_1 = plot.density_sp1 * ((25/plot.qm_dbh_sp1) ** (-1.8491 - 0.0035 * M))
            else:
                SDI_1 = 0
            plot.add_value('REINEKE_SP1', SDI_1)

            if plot.qm_dbh_sp2 != 0:
                SDI_2 = plot.density_sp2 * ((25/plot.qm_dbh_sp2) ** (-1.5559 - 0.0095 * M))
            else:
                SDI_2 = 0
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(old_tree.specie) == int(plot.id_sp1):  # specie 1 condition
                ...
                bai5_sp1 = math.exp(-1.6673 - 0.0363 * old_tree.dbh + 1.9709 * math.log(old_tree.dbh) - 0.0252 * plot.qm_dbh - 0.0181 * old_tree.basal_area_intrasp - 0.0283 * old_tree.basal_area_intersp - 0.0063 * old_tree.bal + 0.0581 * M)
                ...
                beta0 = -0.0172 * new_tree.bal + 0.0734 * plot.qm_dbh + 1.5234 * plot.sp1_proportion
                h_sp1 = 1.3 + (beta0 * (1/new_tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_h - 1.3)) ** 0.5) ** (-2)
                ...
                bai5_sp2 = math.exp(-1.0381 - 0.0373 * old_tree.dbh + 1.9721 * math.log(old_tree.dbh) + 0.0262 * old_tree.height - 0.0205 * plot.qm_dbh - 0.0343 * old_tree.basal_area_intrasp + 0.0238 * M)
                new_tree.sum_value("basal_area", bai5_sp2)  # update basal area (cm2) 

                new_tree.add_value('dbh', 2*math.sqrt(new_tree.basal_area/math.pi))  # upload dbh (cm)

                beta0 = 1.2129 - 0.0402 * new_tree.bal + 0.0431 * plot.qm_dbh_sp2
                h_sp2 = 1.3 + (beta0 * (1/new_tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_h - 1.3)) ** 0.5) ** (-2)
    ...
    def merchantable(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            usage, merch_list = TreeModel.merch_calculation_mix(tree, plot, class_conditions, PhalepensisPpinasterMix)
    ...
    def biomass(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition [Pinus pinaster]
            ...
            elif int(tree.specie) == int(plot.id_sp2):  # specie 2 condition [Pinus halepensis]
