...
# Pure stands of Quercus suber located on Cataluña (Spain)
# Version 02
#
...
class QuercusSuberCataluña(TreeModel):
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Qsuber__cat__v01'  # set the model name to show it at the output
            Model.specie_ifn_id = 46  # Set the model specie ID to mark the trees of different species         
            ...
            Model.valid_prov_reg = '8, 9, Q'  # provenance regions recommended to use the model
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                     Running: Quercus suber model (Cataluña). Plot:', plot.plot_id                   )
        print('#--------------------------------------------------------------------------------------------------#')
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if old_tree.specie == Model.specie_ifn_id:  # specie condition
                ...
                #    Xo = Xo1 # +- Xo2
                #    cork_2 = old_tree.bark*(((1 - math.exp(-0.04*new_tree.tree_age)) / (1 - math.exp(-0.04*old_tree.tree_age)))**((0.57+1.86)/Xo))
                #else:
                #    cork_2 = 0

                #new_tree.add_value('bark', cork_2)
                ...
                Lo = math.log(1 - math.exp(-b1*old_tree.tree_age))
                Xo = 0.5*((math.log(Yo) - b2*Lo))
                Y = Yo*(((1 - math.exp(-b1*new_tree.tree_age)) / (1 - math.exp(-b1*old_tree.tree_age)))**((b2 + b3)/Xo))

                new_tree.sum_value('bark', Y/10)


                new_tree.add_value('dbh_oc', new_tree.dbh + new_tree.bark)
    ...
    def crown(self, tree: Tree, plot: Plot, func):
        ...
        try:  # errors inside that construction will be announced
            ...
            if func == 'initialize':  # if that function is called from initialize, first we must check if those variables are available on the initial inventory
                if tree.lcw == 0 or tree.lcw == '':  # if the tree hasn't height maximum crown-width (m) value, it is calculated
    ...
    def vol(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            if isinstance(tree.bark, float) and isinstance(tree.h_debark, float) and isinstance(tree.dbh_oc, float):
                tree.add_value('bark_vol', (tree.bark/100) * (tree.h_debark*10) * ((tree.dbh / 10)))  # cork fresh volume (dm3)
                #tree.add_value('bark_vol', tree.bark*tree.normal_circumference*tree.h_debark/10)
                tree.add_value('vol', tree.bole_vol + tree.bark_vol)  # volume over bark (dm3)
        ...
        try:  # errors inside that construction will be announced
            ...
            for tree in plot_trees:
                ...
                if tree.specie == Model.specie_ifn_id:  # specie condition
                    ...
                    plot_bark += tree.expan*tree.bark
                    plot_bole_vol += tree.expan*tree.bole_vol
            plot_vol = plot_bark + plot_bole_vol
    ...
    def biomass(self, tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if isinstance(tree.h_debark, float) and isinstance(tree.dbh_oc, float):
