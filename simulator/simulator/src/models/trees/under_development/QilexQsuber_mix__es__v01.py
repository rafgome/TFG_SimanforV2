...
# Mixed stands of Quercus ilex and Quercus suber located on Spain
# Version 01
#
...
class QilexQsuberMix(TreeModel):

    def __init__(self, configuration=None):
        super().__init__(name="QilexQsuberMix", version=1)
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('            Running: Quercus ilex - Quercus suber mixed model (Spain). Plot:', plot.plot_id        )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            # Specie 1 = Quercus ilex (45)
            # Specie 2 = Quercus suber (46) 
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        beta0 = -0.2571 * plot.qm_dbh_sp1 - 0.0323 * plot.sp1_proportion + 0.2214 * M + 1.0962 * plot.dominant_h_sp1
                        beta1 = -7.2568
                        h_sp1 = 1.3 + beta0 * math.exp(beta1 / tree.dbh)
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        beta0 = 1.4835 - 0.0386 * tree.bal + 0.0578 * plot.qm_dbh + 0.8232 * plot.sp2_proportion
                        h_sp2 = 1.3 + ((beta0 * (1/tree.dbh - 1/plot.dominant_dbh)) + ((1/(plot.dominant_h - 1.3)) ** 0.5)) ** (-2)
            ...
            SDImax_1 = math.exp(12.379 + ((-2.1959 + 0.0054 * M) * math.log(25)))
            plot.add_value('REINEKE_MAX_SP1', SDImax_1)
            SDImax_2 = math.exp(13.048 + ((-2.2457 + 0.0061 * M) * math.log(25)))
            plot.add_value('REINEKE_MAX_SP2', SDImax_2)


            # Calculate both SDI values
            if plot.qm_dbh_sp1 != 0:
                SDI_1 = plot.density_sp1 * ((25/plot.qm_dbh_sp1) ** (-2.1959 + 0.0054 * M))
            else:
                SDI_1 = 0
            plot.add_value('REINEKE_SP1', SDI_1)

            if plot.qm_dbh_sp2 != 0:
                SDI_2 = plot.density_sp2 * ((25/plot.qm_dbh_sp2) ** (-2.2457 + 0.0061 * M))
            else:
                SDI_2 = 0
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(old_tree.specie) == int(plot.id_sp1):  # specie 1 condition
                ...
                bai5_sp1 = math.exp(-0.8205 - 0.0294 * old_tree.dbh + 1.3885 * math.log(old_tree.dbh) + 0.1355 * old_tree.height - 0.0364 * old_tree.basal_area_intrasp)
                ...
                beta0 = -0.2571 * plot.qm_dbh_sp1 - 0.0323 * plot.sp1_proportion + 0.2214 * M + 1.0962 * plot.dominant_h_sp1
                beta1 = -7.2568
                h_sp1 = 1.3 + beta0 * math.exp(beta1 / new_tree.dbh)
                ...
                bai5_sp2 = math.exp(0.1117 - 0.0093 * old_tree.dbh + 0.9492 * math.log(old_tree.dbh) + 0.0924 * old_tree.height + 0.0185 * plot.qm_dbh - 0.0296 * old_tree.basal_area_intrasp - 0.0141 * old_tree.basal_area_intersp)
                new_tree.sum_value("basal_area", bai5_sp2)  # update basal area (cm2) 
                dbh = old_tree.dbh
                new_tree.add_value('dbh', 2*math.sqrt(new_tree.basal_area/math.pi))  # upload dbh (cm)

                beta0 =  1.4835 - 0.0386 * new_tree.bal + 0.0578 * plot.qm_dbh + 0.8232 * plot.sp2_proportion
               
                #h_sp2_a = 1.3 + (beta0*(1/dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_h - 1.3))**0.5)**(-2)
                #h_sp2_b = 1.3 + (beta0*(1/new_tree.dbh - 1/plot.dominant_dbh) + (1/(plot.dominant_h - 1.3))**0.5)**(-2)

                #h_2 = old_tree.height*h_sp2_b/h_sp2_a
                #print(h_sp2_a, h_sp2_b, h_2)

                #print(plot.dominant_h, h_sp2)
    ...
    def merchantable(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            usage, merch_list = TreeModel.merch_calculation_mix(tree, plot, class_conditions, QilexQsuberMix)
    ...
    def vol(self, tree: Tree, plot: Plot):
        ...
        Volume under bark equation (Qilex):
        ...
        Volume under bark equation (Qsub):
            Doc.: Amaral J, Tomé M (2006). Equações para estimação do volume e biomassa de duas espécies de carvalhos: Quercus suber e Quercus ilex. Publicações do GIMREF, 1-21
            Ref.: Amaral and Tomé (2006)          
        """
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition
                ...
                vub = 0.000116*(tree.dbh**1.983649)  # m3
                tree.add_value('bole_vol', vub)
    ...
    def biomass(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition [Quercus ilex]
            ...
            elif int(tree.specie) == int(plot.id_sp2):  # specie 2 condition [Quercus suber]
