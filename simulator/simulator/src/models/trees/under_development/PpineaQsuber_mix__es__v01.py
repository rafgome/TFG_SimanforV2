...
# Mixed stands of Pinus pinea and Quercus suber located on Spain
# Version 01
#
...
class PpineaQsuberMix(TreeModel):

    def __init__(self, configuration=None):
        super().__init__(name="PpineaQsuberMix", version=1)
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('            Running: Pinus pinea - Quercus suber mixed model (Spain). Plot:', plot.plot_id        )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        beta0 = 2.1130 - 0.0258 * tree.bal - 0.046 * plot.qm_dbh_sp1 + 1.2023 * plot.sp1_proportion
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        beta0 = 0.3620 * plot.qm_dbh_sp2 + 0.8264 * plot.sp2_proportion
            ...
            SDImax_1 = math.exp(13.304 + ((-2.2518 + 0.0077 * M) * math.log(25)))
            ...
            if plot.qm_dbh_sp1 != 0:
                SDI_1 = plot.density_sp1 * ((25/plot.qm_dbh_sp1) ** (-2.2518 + 0.0077 * M))
            else:
                SDI_1 = 0
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(old_tree.specie) == int(plot.id_sp1):  # specie 1 condition
                ...
                bai5_sp1 = math.exp(0.6606 + 1.3249 * math.log(old_tree.dbh) - 0.0427 * old_tree.basal_area_intrasp - 0.0187 * old_tree.basal_area_intersp)
                ...
                beta0 = 2.1130 - 0.0258 * new_tree.bal - 0.046 * plot.qm_dbh_sp1 + 1.2023 * plot.sp1_proportion
                ...
                bai5_sp2 = math.exp(0.9728 + 0.811 * math.log(old_tree.dbh) + 0.1237 * old_tree.height - 0.0282 * plot.basal_area - 0.0244 * M)
    ...
    def merchantable(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            usage, merch_list = TreeModel.merch_calculation_mix(tree, plot, class_conditions, PpineaQsuberMix)
    ...
    def biomass_plot(self, plot: Plot, plot_trees):
        ...
        try:  # errors inside that construction will be announced
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition [Pinus pinea]
