...
# Mixed stands of Pinus pinaster and Pinus sylvestris located on Spain
# Version 01
#
...
class PpinasterPsylvestrisMix(TreeModel):

    def __init__(self, configuration=None):
        super().__init__(name="PpinasterPsylvestrisMix", version=1)
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('            Running: Pinus pinaster - Pinus sylvestris mixed model (Spain). Plot:', plot.plot_id        )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:  # errors inside that construction will be announced
            ...
            # Specie 1 = Pinus pinaster (26)
            # Specie 2 = Pinus sylvestris (21) 
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if int(tree.specie) == int(plot.id_sp1):  # specie 1 condition
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        beta0 = 0.1674 - 0.0846 * plot.sp2_proportion
                        beta1 = 5.7848
                        h_sp2 = 1.3 + (plot.dominant_height - 1.3) * math.exp(beta0 * (1 - plot.dominant_dbh/tree.dbh) + beta1 * (1/plot.dominant_dbh - 1/tree.dbh))
            ...
            SDImax_2 = math.exp(12.579 + ((-1.7462 + 0.0007 * M) * math.log(25)))
            ...
            if plot.qm_dbh_sp2 != 0:
                SDI_2 = plot.density_sp2 * ((25/plot.qm_dbh_sp2) ** (-1.7462 + 0.0007 * M))
            else:
                SDI_2 = 0
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            if int(old_tree.specie) == int(plot.id_sp1):  # specie 1 condition
                ...
                bai5_sp1 = math.exp(-0.6012 - 0.0249 * old_tree.dbh + 2.1674 * math.log(old_tree.dbh) - 0.0419 * plot.qm_dbh - 0.0149 * plot.basal_area - 0.0033 * old_tree.bal_intrasp)
                ...
                beta0 = 2.7801 - 0.0132 * new_tree.bal - 0.0203 * plot.qm_dbh
                ...
                bai5_sp2 = math.exp(-1.4576 - 0.0393 * old_tree.dbh + 2.3275 * math.log(old_tree.dbh) + 0.0225 * old_tree.height - 0.0441 * plot.qm_dbh - 0.0205 * old_tree.basal_area_intrasp - 0.013 * old_tree.basal_area_intersp - 0.0016 * old_tree.bal_intrasp + 0.01 * M)
                ...
                beta0 = 0.1674 - 0.0846 * plot.sp2_proportion
                beta1 = 5.7848
                h_sp2 = 1.3 + (plot.dominant_h - 1.3) * math.exp(beta0 * (1 - plot.dominant_dbh/new_tree.dbh) + beta1 * (1/plot.dominant_dbh - 1/new_tree.dbh))
    ...
    def merchantable(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            usage, merch_list = TreeModel.merch_calculation_mix(tree, plot, class_conditions, PpinasterPsylvestrisMix)
    ...
    def biomass(self, tree: Tree, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            elif int(tree.specie) == int(plot.id_sp2):  # specie 2 condition [Pinus sylvestris]
