...
# Pure stands of Pinus halepensis located on Aragón (Spain)
# Version 01
#
...
class PinusHalepensisAragon(TreeModel):
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Phalepensis__aragon__v01'  # set the model name to show it at the output
            Model.specie_ifn_id = 24  # Set the model specie ID to mark the trees of different species        
            Model.exec_time = 10  # recommended executions time to use that model
            Model.aplication_area = 'Aragón (Spain)'  # area recommended to use the model; just write 'none' if it is not defined yet
            Model.valid_prov_reg = '3, 4, 5, 6'  # provenance regions recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/arbol/Phalepensis_ar_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/arbol/Phalepensis_ar_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                   Running: Pinus halepensis model (Aragón). Plot:', plot.plot_id                    )
        print('#--------------------------------------------------------------------------------------------------#')

        try:  # errors inside that construction will be announced

            self.model_info()
            plot.add_value('REINEKE_VALUE', -1.8186)  # r constant value of SDI to the species of the model (-1.605 as default)             
            ...
            bal: float = 0
            global Ghl
            Ghl = 0  # variable needed to calculate the basal area of only Pinus halepensis, needed at ingrowth function
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if tree.specie == Model.specie_ifn_id:  # specie condition
                    ...
                    Ghl += tree.ba_ha  # variable needed to calculate the basal area of only Pinus halepensis, needed at ingrowth function
    ...
    def survival(self, time: int, plot: Plot, tree: Tree):
        ...
        if tree.specie == Model.specie_ifn_id:  # specie condition
            psurvive10 = 1/(1 + math.exp(-6.5934 + 0.0305*plot.basal_area + 5.6845*tree.bal/plot.basal_area - 8.1523*plot.hart))  # 10 years period change
    ...
    def ingrowth(self, time: int, plot: Plot):
        ...
        if plot.basal_area == 0 or plot.basal_area == '' or Model.slope == 0 or Model.slope == '':  # to not obtain an error because of math.log(0)
            return 0
        else:

            try:

                G = plot.basal_area  # stand basal area of all trees (m2/ha)
                global Ghl  # stand basal area of only Pinus halepensis trees (m2/ha)
                SLO = Model.slope  # slope(%)
                ...
                # ING = number of tree per ha
                N_ingrowth = beta1 + beta2*math.log(G) + beta3/G + beta4*(Ghl/G) + beta5*math.log(SLO)  # 10 years period

                G_ingrowth = N_ingrowth*plot.basal_area/plot.density
    ...
    def taper_over_bark(self, tree: Tree, hr: float):
        ...
        try:  # errors inside that construction will be announced
            ...
            # b)
            E = 100 * tree.height / tree.dbh  # TODO: on the new paper doesnt appear 100*
    ...
    def taper_under_bark(self, tree: Tree, hr: float):
        ...
        ¡IMPORTANT! It is not used math.exp because of calculation error, we use the number "e" instead, wrote by manually.
        """
        # TODO: faltaría meter bark thickness y con esto se completaría el modelo
        # TODO: Rodríguez-Puerta et al., 2023. Individual-tree growth system for even-aged Aleppo pine plantations in
        # Aragón, Spain
            ...
            if plot.hart != 0 and plot.hart != '':
                if plot.basal_area != '' and plot.basal_area != 0:
                    if func == 'initialize':  # if that function is called from initialize, first we must check if those variables are available on the initial inventory
                        if tree.hcb == 0 or tree.hcb == '':  # if the tree hasn't basal crown (m) value, it is calculated
                            tree.add_value('hcb', tree.height / (1 + math.exp(-0.82385 + 4.039408*plot.hart*  # TODO: el 0.01 que multiplica a hart no está en el paper
