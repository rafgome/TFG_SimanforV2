...
class PinusSylvestrisSISC(TreeModel):


    def __init__(self, configuration=None):
        super().__init__(name="Pinus sylvestris - Sistema Ibérico y Sistema Central", version=2)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Psylvestris_LIDAR__sisc__v01'  # set the model name to show it at the output
            ...
            Model.aplication_area = 'Sistema Ibérico y Sistema Central (Spain)'  # area recommended to use the model; just write 'none' if it is not defined yet
    ...
    def initialize(self, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            for tree in plot_trees:  # for each tree...
                ...
                TreeModel.hegyi(plot, tree, plot_trees, plot.hegyi_radius)  # activate Hegyi calculation
    ...
    def survival(self, time: int, plot: Plot, tree: Tree):
        ...
        #else:
        #    return 1
        return 1  # cancel this function to reduce noise on the analisis
    ...
    def ingrowth(self, time: int, plot: Plot):
        ...
        #else:
        #    return 0
        return 0  # cancel this function to reduce noise on the analisis
    ...
    def ingrowth_distribution(self, time: int, plot: Plot, area: float):
        ...
        return None  # cancel this function to reduce noise on the analisis
    ...
    def merchantable(self, tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            usage, merch_list = TreeModel.merch_calculation(tree, class_conditions, PinusSylvestrisSISC)
    ...
    def crown(self, tree: Tree, plot: Plot, func):
        ...
        That function is run by initialize and update_model functions.
        Crown equations (cr and lcw):
            Doc.: Lizarralde I (2008). Dinámica de rodales y competencia en las masas de pino silvestre (Pinus sylvestris L.) y pino negral (Pinus pinaster Ait.) de los Sistemas Central e Ibérico Meridional. Tesis Doctoral. 230 pp           
            Ref.: Lizarralde 2008
        Crown equations (hlcw, hcb, cpa and crown_vol):
            Doc.: 
            Ref.: 
        """
        ...
        try:  # errors inside that construction will be announced
            ...
            TH = tree.height
            CI = tree.hegyi
            RatioBALpine = tree.bal_ratio_intrasp_hegyi
            BApine = tree.g_intrasp_hegyi
            BAtotal = tree.g_total_hegyi
            BAL = tree.bal
            ...
            if func == 'initialize':  # if that function is called from initialize, first we must check if those variables are available on the initial inventory
                if tree.hlcw == 0 or tree.hlcw == '':  # if the tree hasn't height maximum crown-width (m) value, it is calculated                          
                    # this equation was prepared to be used with a radius = 10 to calculate Hegyi index
                    TreeModel.hegyi(plot, tree, plot_trees, 10)  # activate Hegyi calculation
                    MCWH = TH/(1 + math.exp(-0.11*CI - 0.13*RatioBALpine))                    
                if tree.hcb == 0 or tree.hcb == '':
                    # this equation was prepared to be used with a radius = 5 to calculate Hegyi index
                    TreeModel.hegyi(plot, tree, plot_trees, 5)  # activate Hegyi calculation
                    CBH = MCWH/(1 + math.exp(-22.9*(BAtotal/tree.dbh) - 0.1*BApine - 0.41*RatioBALpine))                    
            else:       
                # this equation was prepared to be used with a radius = 10 to calculate Hegyi index
                TreeModel.hegyi(plot, tree, plot_trees, 10)  # activate Hegyi calculation     
                MCWH = TH/(1 + math.exp(-0.11*CI - 0.13*RatioBALpine))                   
            ...
            tree.add_value('hcb', CBH)
            tree.add_value('hlcw', MCWH)
            ...
            if tree.hcb != '' and tree.hcb != 0:
                ...
                tree.add_value('cr', 0)
                tree.add_value('lcw', 0)

            CPA = math.exp(0.07*tree.dbh + 0.02*BAtotal - 0.1*CI)
            tree.add_value('cpa', CPA)  # Crown Projection Area

            CV = 0.0015*(tree.dbh**2)*tree.height + 0.44*BAtotal - 2.35*CI
            tree.add_value('crown_vol', CV)  # Crown Volume
    ...
    def canopy(self, plot: Plot, plot_trees):
        ...
        try:  # errors inside that construction will be announced
            ...
            plot_expan = plot_lcw = plot_lcw2 = plot_fcc = plot_canopy_vol = 0
            ...
            for tree in plot_trees:  # for each tree, we are going to add the simple values to the plot value
                ...
                if tree.specie == Model.specie_ifn_id:  # specie condition
                    ...
                    plot_canopy_vol += tree.crown_vol*tree.expan
            ...
            plot.add_value('CANOPY_VOL', plot_canopy_vol)            
