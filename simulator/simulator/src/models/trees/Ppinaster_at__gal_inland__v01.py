...
# Pure stands of Pinus pinaster atlantica located on Galicia inland and León (Spain)
# Version 01
#
...
class PinusPinasterGaliciaInland(TreeModel):

    def __init__(self, configuration=None):
        super().__init__(name="Pinus pinaster atlantica inland", version=1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Ppinaster_at__gal_inland__v01'  # set the model name to show it at the output
            ...
            Model.aplication_area = 'Galicia $ León interior/inland (Spain)'  # area recommended to use the model; just write 'none' if it is not defined yet
            Model.valid_prov_reg = '1b'  # provenance regions recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/arbol/Ppinaster_atlantica_inland_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/arbol/Ppinaster_atlantica_inland_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('           Running: Pinus pinaster atlantica inland model (Galicia). Plot:', plot.plot_id            )
        print('#--------------------------------------------------------------------------------------------------#')
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if tree.specie == Model.specie_ifn_id:  # specie condition
                    ...
                    if tree.height == 0 or tree.height == '':  # if the tree hasn't height (m) value, it is calculated
                        I = 0  # dummy variable which assumes a value of 0 for the interior ecoregion and 1 for the coastal ecoregion.
                        h = (1.3**(1.894 + 1.469*I) + (plot.dominant_h**(1.894 + 1.469*I) - 1.3**(1.894 + 1.469*I)) * ((1 - math.exp(-(0.04611 - 0.04734*I)*tree.dbh)) / (1 - math.exp(-(0.04611 - 0.04734*I)*plot.dominant_dbh))))**(1/(1.894 + 1.469*I))
    ...
    def merchantable(self, tree: Tree):
        ...
        try:  # errors inside that construction will be announced
            ...
            usage, merch_list = TreeModel.merch_calculation(tree, class_conditions, PinusPinasterGaliciaInland)
    ...
    def crown(self, tree: Tree, plot: Plot, func):
        ...
        That function is run by initialize and update_model functions.
        Crown equations:
            Doc.: Crecente-Campo F, Álvarez-González JG, Castedo-Dorado F, Gómez-García E, Diéguez-Aranda U (2013). Development of crown profile models for Pinus pinaster Ait. and Pinus sylvestris L. in northwestern Spain. Forestry, 86(4), 481-491
            Ref.: Crecente-Campo et al. 2013
        """
        ...
        try:  # errors inside that construction will be announced
            ...
            if tree.hcb != 0 and tree.hcb != '':
                ...
                HT = tree.height
                d = tree.dbh
                if d != 0:  # to not obtain an error because of math.log(0)            
                ...
                tree.add_value('lcw', 0)  # maximum crown-width (m)
                tree.add_value('hlcw', 0)  # height maximum crown-width (m)
                tree.add_value('cr', 0)  # crown ratio (%)                
...
PinusPinasterGaliciaInland.vars()