...
# Pure stands of Pinus nigra located on Cataluña (Spain)
# Version 01
#
...
class PinusNigraCataluña(TreeModel):
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Pnigra__cat__v01'  # set the model name to show it at the output
            ...
            Model.valid_prov_reg = '3, 4, 5'  # provenance regions recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/arbol/Pnigra_cat_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/arbol/Pnigra_cat_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                    Running: Pinus nigra model (Cataluña). Plot:', plot.plot_id                      )
        print('#--------------------------------------------------------------------------------------------------#')

        try:  # errors inside that construction will be announced

            self.model_info()
            plot.add_value('REINEKE_VALUE', -1.6824)  # r constant value of SDI to the species of the model (-1.605 as default)            
            ...
            # if plot.dominant_h != 0:  # to not obtain an error because of math.log(0)            
            ...
            global Gnig  # variable needed to calculate the basal area of Pnigra (no more species)
            Gnig = 0
            ...
            for tree in plot_trees:  # for each tree...
                ...
                if tree.specie == Model.specie_ifn_id:  # specie condition
                    ...
                    Gnig += tree.expan*tree.basal_area
    ...
    def survival(self, time: int, plot: Plot, tree: Tree):
        ...
        if tree.specie == Model.specie_ifn_id:  # specie condition
            ...
            beta1 = -0.0400
            beta2 = 6.9900
            ba_survival = 1 / (1 + math.exp( - (beta0 + beta1*tree.bal + beta2*(tree.height / plot.dominant_h))))
            ...
            # beta2 = 0.094
            # beta3 = -0.146
            # ba_survival = 1 / (1 + math.exp( - (beta0 + (beta1*tree.bal)/(math.log(tree.dbh + 1)) + beta2*plot.basal_area + beta3*(plot.altitud / 100))))
    ...
    def growth(self, time: int, plot: Plot, old_tree: Tree, new_tree: Tree):
        ...
        try:  # errors inside that construction will be announced        
            ...
            if old_tree.specie == Model.specie_ifn_id:  # specie condition
                ...
                # b) dbhg5 la descarto por las variables que usa (masa mixta con Psylvestris, y BALthin no se puede conseguir) y R2 < 0.2; htg5: R2 < 0.5
                # beta1 = 26.2556
                # beta2 = 29.2372
                # beta3 = -22.1194
    ...
    def ingrowth(self, time: int, plot: Plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            if Area.continentality[plot.plot_id] == 0 or Area.continentality[plot.plot_id] == '':
                return 0  # return 0 if variables are not available at the input
            elif Area.altitude[plot.plot_id] == 0 or Area.altitude[plot.plot_id] == '':
                return 0  # return 0 if variables are not available at the input
            else:
                CON = Area.continentality[plot.plot_id]  # continentality (linear distance to the Mediterranean Sea, km)
                ELE = Area.altitude[plot.plot_id]/100  # elevation (100 m)
            G = plot.basal_area  # stand basal area (m2/ha)

            global Gnig  # stand basal area Pnigra (m2/ha)
            Gnig = Gnig/10000
            ...
            # N_ingrowth = nº trees/ha
            N_ingrowth = beta0 + beta1*G + beta2*(Gnig/G) + beta3*(CON/ELE)  # 10 years period
    ...
    def update_model(self, time: int, plot: Plot, trees: list):
        ...
        try:  # errors inside that construction will be announced
            ...
            if plot.year != '':
                plot.add_value('YEAR', plot.year + time)
    ...
    def mushrooms(self, plot):
        ...
        try:  # errors inside that construction will be announced
            ...
            syl = 0  # dummy variable: 1 to Psylvestris stands, 0 to Pnigra stands
            planted = 0  # dummy variable: 1 to Psylvestris planted stands, 0 to other cases

            if G != 0 and G != '':
                if Ele != 0 and Ele != '':
                    if Slo != 0 and Slo != '':  # to not obtain an error because of math.log(0)
                        # Edible mushrooms
            ...
            # Marketed Lactarius
            #plot.add_value('MARKETED_LACTARIUS', 0)  # production of Lactarius group deliciosus (kg/ha*year)
