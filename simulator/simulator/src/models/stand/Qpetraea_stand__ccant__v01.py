...
# Stand Static model, developed to Quercus petraea at the Cordillera Cantábrica (Spain)


# Pure stands of Quercus petraea located on Cordillera Cantábrica (Spain)
...
class QuercusPetraeaCCantStand(StandModel):

    def __init__(self, configuration=None):
        super().__init__(name="QuercusPetraeaCCantStand", version=1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.specie_ifn_id = 42  # Set the model specie ID to mark the trees of different species
            Model.exec_time = 999  # recommended executions time to use that model
            Model.aplication_area = 'Cordillera Cantábrica (Spain)'  # area recommended to use the model; just write 'none' if it is not defined yet
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/masa/Qpetraea_cant_stand_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/masa/Qpetraea_cant_stand_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        Density, quadratic diameter, dominant height, basal area, mean diameter and volume equations:
            Doc.: Reque, J (2004). Modelo estático de producción para masas naturales de Quercus petraea en la Cordillera Cantábrica. Aplicación de la base de datos del segundo inventario forestal nacional. Cuadernos de la Sociedad Española de Ciencias Forestales, 18: 185-192
            Ref.: Reque, 2004
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('         Running: Quercus petraea stand model (Cordillera Cantábrica). Plot:', plot.plot_id          )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            if Area.specie_ifn_id[plot.plot_id] != Model.specie_ifn_id:
                print()
                print('BE CAREFUL! You are trying to run that model of Quercus petraea with an inventory of another specie.')
    ...
    #-------------------- Plot data calculation after having all the needed input variables -------------------------#
            ...
            # Quadratic Mean Diameter and Density
            if plot.density != 0 and plot.density != "":
                dg = math.exp((11.7025 - math.log(plot.density))/1.61578) 
                plot.add_value('QM_DBH', dg)
            else:
                N = math.exp(11.7025 - 1.61578*math.log(plot.qm_dbh)) 
                plot.add_value('DENSITY', N)

            # Dominant Height
            H0 = math.exp((2.451 - 0.241*math.log(plot.density) - math.log(plot.qm_dbh))/(-0.859)) 
            plot.add_value('DOMINANT_H', H0)

            # Basal Area
            G = math.exp(- 4.55 + 0.518*math.log(plot.density) + 1.718*math.log(plot.dominant_h))
            plot.add_value('BASAL_AREA', G)

            # Mean Diameter
            mean_dbh = math.exp(2.383 - 0.241*math.log(plot.density) + 0.837*math.log(plot.dominant_h))
            ...
            V = math.exp( - 0.297 + 0.987*math.log(plot.basal_area) + 0.732*math.log(plot.dominant_h))
            ...
            if plot.qm_dbh != 0:
                ...
                # SDI = math.exp(-4.142+(0.971*math.log(plot.dominant_h))-(math.log(plot.vol)/1.094))
                # plot.add_value('REINEKE', SDI)
    ...
    def growth(self, old_plot: Plot, new_plot: Plot, time: int):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('         Running: Quercus petraea stand model (Cordillera Cantábrica). Plot:', old_plot.plot_id          )
        print('#--------------------------------------------------------------------------------------------------#')
    ...
    def harvest(self, old_plot: Plot, new_plot: Plot, cut_criteria, volume, time, min_age, max_age):
        ...
        try:
            ...
            if cut_criteria == CUTS_DICT['PERCENTOFTREES']:
                ...
                if N != 0:
                    dg = math.exp((11.7025 - math.log(N))/1.61578)  # Quadratic Mean Diameter
                    H0 = math.exp((2.451 - 0.241*math.log(N) - math.log(dg))/(-0.859))
                    G = math.exp(-4.55+(0.518* math.log(N))+(1.718*math.log(H0)))
                    V = math.exp(-0.297+(0.987*math.log(G))+(0.732*math.log(H0)))  # equation to calculate volume
                else:
                    G = dg = V = 0
                ...
                if G != 0:
                    H0 = old_plot.dominant_h
                    N = math.exp(-4.55+(math.log(G)/0.518)+(1.718* math.log(H0)))  #vai pressupor que o H0=H1?
                    dg = math.exp(11.7025-(math.log(N))/1.61578)  # Quadratic Mean Diameter
                    V = math.exp(-0.297+(0.987*math.log(G))+(0.732*math.log(H0))) # equation to calculate volume
                ...
                if V != 0:
                    H0 = old_plot.dominant_h
                    G = math.exp(-0.297+(math.log(VCC)/0.987)+(0.732*math.log(H0)))  # equation to calculate stand basal area
                    N = math.exp(-4.55+(math.log(G)/0.518)+(1.718* math.log(H0)))  #vai pressupor que o H0=H1?
                    dg = math.exp(11.7025-(math.log(N)/1.61578)) 
            ...
            new_plot.add_value('DOMINANT_H', H0)  
            ### Dm
            #new_plot.add_value('MEAN_DBH', Dm)     
