...
# Pure stands of Pinus sylvestris located on High Ebro Basin (Spain)
# Version 01
#
...
class PinusSylvestrisHighEbroBasinStand(StandModel):

    def __init__(self, configuration=None):
        super().__init__(name="Pinus sylvestris High Ebro Basin stand model", version=1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Psylvestris_stand__High_Ebro_Basin__v01'  # set the model name to show it at the output
            ...
            Model.aplication_area = 'Alto Valle del Ebro/High Ebro Basin (Spain)'  # area recommended to use the model
            Model.valid_prov_reg = '2'  # provenance regions recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/masa/Psylvestris_stand_ebro_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/masa/Psylvestris_stand_ebro_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        Site Index equation:
            Doc.: Bravo F (1998). Modelo de producción para Pinus sylvestris L. en el Alto Valle del Ebro
            Ref.: Bravo, 1998       
        Dominant Height equation:
            Doc.: Bravo F (1998). Modelo de producción para Pinus sylvestris L. en el Alto Valle del Ebro
            Ref.: Bravo, 1998
        Quadratic mean diameter equation:
            Doc.: Bravo F, Montero G (2003). High-grading effects on Scots pine volume and basal area in pure stands in northern Spain. Annals of forest science, 60(1), 11-18
            Ref.: Bravo and Montero, 2003   
        Basal area equation:
            Doc.: Bravo F, Montero G (2003). High-grading effects on Scots pine volume and basal area in pure stands in northern Spain. Annals of forest science, 60(1), 11-18
            Ref.: Bravo and Montero, 2003     
        Volume equation:
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('              Running: Pinus sylvestris stand model (High Ebro Basin). Plot:', plot.plot_id          )
        print('#--------------------------------------------------------------------------------------------------#')
...
#-------------------- Plot data calculation after having all the needed input variables -------------------------#
            ...
            # Site Index            
            comp= 1  # variable to check the accuracy we want at the next loop
            S_0 = 1.85  # SI that we will make to iterate until the accuracy of the calculation stay at the limit suggested (dam)
            t = plot.age/10  # stand age (decades)
            H = plot.dominant_h/10  # dominant height (dam)
            while comp > 0.00001 or comp < 0:  # loop to calculate Site Index (dam)
                SI = math.exp((math.log(H/(1.06*((1 - math.exp(-0.45*t))**(3.01*(S_0**-0.27))))))/0.84)
                comp = S_0 - SI
                S_0 = SI

            plot.add_value('REF_SI_AGE', 100)  # the reference age is inside the equation calculation
            plot.add_value('SI', SI*10)  # store site index value in m

            # Dominant Height
            si = plot.si/10  # site index (decades)
            t = plot.age/10  # stand age (decades)
            Ho = 1.06*(si**0.84)*((1- math.exp(-0.45*t))**(3.01*(si**-0.27)))  # dominant height (decades)
            plot.add_value('DOMINANT_H', Ho*10)  # store dominant height value in m

            # Quadratic Mean Diameter:          
            a0 = 43.791
            a1 = -0.270
            a2 = 0.426
            dg = a0*(plot.density**a1)*(plot.dominant_h**a2)
            ...
            b1 = -0.107
            b2 = 1.116
            G = b0*(plot.density**b1)*(plot.reineke**b2)
            ...
            c1 = 0.913
            c2 = 0.413
            V = c0*(plot.reineke**c1)*(plot.dominant_h**c2) - 215.069 + 15.685*plot.dominant_h
    ...
    def growth(self, old_plot: Plot, new_plot: Plot, time: int):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('              Running: Pinus sylvestris stand model (High Ebro Basin). Plot:', old_plot.plot_id          )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            # Dominant Height
            si = old_plot.si/10  # site index (decades)
            t = new_age/10  # new age calculation (decades)
            H = 1.06*(si**0.84)*((1- math.exp(-0.45*t))**(3.01*(si**-0.27)))  # dominant age (decades)
            new_plot.add_value('DOMINANT_H', H*10)  # store dominant age value (years)
            ...
            a1 = -0.270
            a2 = 0.426
            dg = a0*(new_plot.density**a1)*(new_plot.dominant_h**a2)
            ...
            b1 = -0.107
            b2 = 1.116
            G = b0*(new_plot.density**b1)*(new_plot.reineke**b2)
            ...
            c1 = 0.913
            c2 = 0.413
            V = c0*(new_plot.reineke**c1)*(new_plot.dominant_h**c2) - 215.069 + 15.685*new_plot.dominant_h
    ...
    def harvest(self, old_plot: Plot, new_plot: Plot, cut_criteria, volume, time, min_age, max_age):
        ...
        try:
            ...
            # Parameters for Quadratic Mean Diameter, Volume and Basal Area equations       
            ...
            if cut_criteria == CUTS_DICT['PERCENTOFTREES']:
                ...
                if N != 0:
                    dg = a0*(N**a1)*(new_plot.dominant_h**a2) 
                    # Reineke Index
                    if dg != 0:
                        SDI = N * pow(25/dg, old_plot.reineke_value)  # Reineke density index
                    G = b0*(N**b1)*(SDI**b2)
                    V = c0*(SDI**c1)*(new_plot.dominant_h**c2) - 215.069 + 15.685*new_plot.dominant_h
                else:
                    G = dg = V = SDI = 0
                ...
                if G != 0:
                    N = ((G*(25**(1.75*b2)))/(b0*(a0**(1.75*b2))*(old_plot.dominant_h**(a2*b2*1.75))))**(1/(b1 + b2*(1 + 1.75*a1)))  # equation created using dg, G and Reineke
                    dg = a0*(N**a1)*(old_plot.dominant_h**a2)  # quadratic mean diameter
                    SDI = N * pow(25/dg, old_plot.reineke_value)  # Reineke density index
                    V = c0*(SDI**c1)*(new_plot.dominant_h**c2) - 215.069 + 15.685*new_plot.dominant_h
                else:
                    N = dg = V = SDI = 0
                ...
                if V != 0:
                    SDI = ((V + 215.069 - 15.685*new_plot.dominant_h)/(c0*(new_plot.dominant_h**c2)))**(1/c1)
                    N = SDI / (pow(25/old_plot.qm_dbh, old_plot.reineke_value))
                    dg = a0*(N**a1)*(new_plot.dominant_h**a2) 
                    G = b0*(N**b1)*(SDI**b2)
                else:
                    G = dg = N = SDI = 0

            #print(dg, old_plot.basal_area, G, V, N, Reineke, old_plot.dominant_h)
            ...
            # Reineke Index
            #if new_plot.qm_dbh != 0:
            #    SDI = new_plot.density*pow(25/new_plot.qm_dbh, -1.75)  # Reineke density index
            #else:
            #    SDI = 0
            new_plot.add_value('REINEKE', SDI)  # store Reineke index in database
...
PinusSylvestrisHighEbroBasinStand.vars()