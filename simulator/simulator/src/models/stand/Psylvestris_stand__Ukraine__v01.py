...
# Pure stands of Pinus sylvestris located on Ukraine
# Version 01
#
# Model transcripted by Orest Pinak in a internship period; adapted and corrected by iuFOR:
...
class PinusSylvestrisUkraine(StandModel):

    def __init__(self, configuration = None):
        super().__init__(name = "Pinus Sylvestris (Ukraine)", version = 1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Psylvestris_stand__Ukraine__v01'  # set the model name to show it at the output
            ...
            Model.aplication_area = 'Ucrania/Ukraine'  # area recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/masa/Psylvestris_stand_ukraine_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/masa/Psylvestris_stand_ukraine_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        Site Index equation:
            Doc.: Каганяк, Ю. (2013). Адаптація системи прогнозу продуктивності соснових деревостанів до умов інтенсивного ведення лісового господарства. In Наукові праці Лісівничої академії наук України (11th ed., pp. 151-156). Retrieved 12 June 2020, from http://www.irbis-nbuv.gov.ua/cgi-bin/irbis_nbuv/cgiirbis_64.exe?I21DBN=LINK&P21DBN=UJRN&Z21ID=&S21REF=10&S21CNR=20&S21STN=1&S21FMT=ASP_meta&C21COM=S&2_S21P03=FILA=&2_S21STR=Nplanu_2013_11_26.
            Ref.: Каганяк, 2013
        Mean Height equation:
            Doc.: Лакида, П., Терентьєв, А., & Алексіюк, І. (2020). In ТЕОРІЯ І ПРАКТИКА ПРИРОДОКОРИСТУВАННЯ. Retrieved 23 May 2020, from http://natureus.org.ua/repec/archive/3_2014/2.pdf.
            Ref.: Лакида et al., 2020
        Mean Diameter equation:
            Doc.: Лакида, П., Терентьєв, А., & Алексіюк, І. (2020). In ТЕОРІЯ І ПРАКТИКА ПРИРОДОКОРИСТУВАННЯ. Retrieved 23 May 2020, from http://natureus.org.ua/repec/archive/3_2014/2.pdf.
            Ref.: Лакида et al., 2020
        Basal Area equation --> standard equation
        Volume equation:
            Doc.: ПРОГНОЗ ПОТЕНЦІЙНОЇ ПРОДУКТИВНОСТІ СОСНОВИХ ТА БУКОВИХ ДЕРЕВОСТАНІВ. (2006), 39-45. Retrieved 23 May 2020, from https://cyberleninka.ru/article/n/prognoz-potentsiynoyi-produktivnosti-sosnovih-ta-bukovih-derevostaniv.
            Ref.: ПРОГНОЗ ПОТЕНЦІЙНОЇ ПРОДУКТИВНОСТІ СОСНОВИХ ТА БУКОВИХ ДЕРЕВОСТАНІВ, 2006
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                Running: Pinus sylvestris stand model (Ukraine). Plot:', plot.plot_id                )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            if Area.specie_ifn_id[plot.plot_id] != Model.specie_ifn_id:
                print()
                print('BE CAREFUL! You are trying to run that model of Pinus sylvestris with an inventory of another specie.')
...
#-------------------- Plot data calculation after having all the needed input variables -------------------------#
            ...
            if plot.si == 0 or plot.si == '':
                ...
                SI = plot.mean_h * (t2**(parA - parB*math.log(t2)) / (plot.age**(parA - parB*math.log(plot.age)))) ** parC  # equation for Site Index
                plot.set_value('SI', SI)  # store Site Index in database
            ...
            parA4 = 0.9874
            parA5 = -3.0183
            mean_h = ((parA0*(1 - math.exp(parA1*plot.age*(1 - math.exp(parA2*plot.age))))**parA3)**parA4) * math.exp(parA5/plot.age)*plot.si  # Average height at plot age for actual site index 
            # base_height it is height in 100 years 
            plot.sum_value('MEAN_H', mean_h)  # store Average height in database
            ...
            parD1 = 0.3443
            parD2 = 0.7616
            mean_dbh = parD0*(plot.age**parD1)*(plot.mean_h**parD2)
            plot.set_value('MEAN_DBH', mean_dbh)  # store Average diameter in database
            ...
            MTBA = math.pi*((plot.mean_dbh/2)**2)  # basal area calculus per each tree
            G = (MTBA/10000)*plot.density
            plot.set_value('BASAL_AREA', G)  # store basal area in database
            ...
            parB3 = 0.2536
            parB4 = 0.0277        
            V = (parB0 + parB1*plot.si)*(plot.mean_h**3) - parB2*math.log(plot.mean_h) + parB3*(math.log(plot.mean_h)**2) - parB4*(math.log(plot.mean_h)**3)  # equation for stand volumen
            plot.set_value('VOL', V)  # store initial stand volume in database

            # Quadratic Mean Diameter:      
            if plot.density != 0:
                MTBA = plot.basal_area*10000/plot.density  # Mean Tree Basal Area
                dg  = 2*math.sqrt(MTBA/math.pi)  # Quadratic Mean Diameter
                plot.set_value('QM_DBH', dg)  # store quadratic mean diameter
    ...
    def growth(self, old_plot: Plot, new_plot: Plot, time: int):
        ...
        Volume equation:
            ...
            Ref.: ПРОГНОЗ ПОТЕНЦІЙНОЇ ПРОДУКТИВНОСТІ СОСНОВИХ ТА БУКОВИХ ДЕРЕВОСТАНІВ, 2006
        Survive equation:
            Doc.: Каганяк, Ю., & Жмурко, С. (2005). Dynamic of basic taxation characteristics of Jack pine forest in dry land. In Збірник науково-технічних праць (15th ed., pp. 44-48). Retrieved 12 June 2020, from https://cyberleninka.ru/article/n/dinamika-osnovnih-taksatsiynih-pokaznikiv-derevostaniv-sosni-banksa-v-suhih-borah/viewer.
            Ref.: Каганяк and Жмурко, 2005
        Quadratic Mean Diameter --> standard equation
        Reineke Index equation (value) --> standard value
        Hart Index equation --> standard equation
        """
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                Running: Pinus sylvestris stand model (Ukraine). Plot:', old_plot.plot_id                )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            parA4 = 0.9874
            parA5 = -3.0183
            mean_h = ((parA0*(1 - math.exp(parA1*old_plot.age*(1 - math.exp(parA2*old_plot.age))))**parA3)**parA4) * math.exp(parA5/old_plot.age)*old_plot.si  # Average height at plot age for actual site index 
            # base_height it is height in 100 years 
            new_plot.sum_value('MEAN_H', mean_h)  # store Average height in database
            ...
            parD1 = 0.3443
            parD2 = 0.7616
            mean_dbh = parD0*(old_plot.age**parD1)*(new_plot.mean_h**parD2)
            new_plot.set_value('MEAN_DBH', mean_dbh)  # store Average diameter in database
            ...
            parC1 = -0.0027
            parC2 = 0.7129
            N = (23157 - parC0*old_plot.si) * (new_age**(parC1*old_plot.si - parC2))  # density calculus; new value of trees per hectare
            new_plot.set_value('DENSITY', N)  # store trees per hectare in database
            ...
            # Basal Area        
            # NO EQUATION AVAILABLE
            MTBA = math.pi*((new_plot.mean_dbh/2)**2)  # basal area calculus per each tree
            G = (MTBA/10000)*new_plot.density
            new_plot.set_value('BASAL_AREA', G)  # store basal area in database
            ...
            parB3 = 0.2536
            parB4 = 0.0277        
            V = (parB0 + parB1*old_plot.si)*(new_plot.mean_h**3) - parB2*math.log(new_plot.mean_h) + parB3*(math.log(new_plot.mean_h)**2) - parB4*(math.log(new_plot.mean_h)**3)  # equation for stand volumen
            new_plot.set_value('VOL', V)  # store total stand volume with bark in database

            # Quadratic Mean Diameter:      
            if new_plot.density != 0:
                MTBA = new_plot.basal_area*10000/new_plot.density  # Mean Tree Basal Area
                dg = 2*math.sqrt(MTBA/math.pi)  # Quadratic Mean Diameter
                new_plot.set_value('QM_DBH', dg)  # store quadratic mean diameter
    ...
    def harvest(self, old_plot: Plot, new_plot: Plot, cut_criteria, volume, time, min_age, max_age):
        ...
        Harvest equations --> own construction
        """
        ...
        try:
            ...
            if cut_criteria == CUTS_DICT['PERCENTOFTREES']:
                ...
                if N != 0:
                    MTBA = math.pi*((old_plot.mean_dbh/2)**2)  # basal area calculus per each tree
                    G = (MTBA/10000)*N  # equation to calculate stand basal area
                    dg  = 2*math.sqrt(MTBA/math.pi)  # Quadratic Mean Diameter

                    # Extrapolation using volume to calculate the new plot density

                    V = old_plot.vol*N/new_plot.density  # equation for stand volumen
                else:
                    G = dg = V = 0
                ...
                if G != 0:
                    ...
                    dg  = 2*math.sqrt(MTBA/math.pi)  # Quadratic Mean Diameter
                    if MTBA != 0:
                        N = G*10000/MTBA  # trees per hectare
                ...
                if V != 0:
                    ...
                    if old_plot.vol != 0:
                        N = V*old_plot.density/old_plot.vol  # trees per hectare
                    G = (MTBA/10000)*N  # equation to calculate stand basal area
                else:
                    G = dg = N = 0
            ...
            parD2 = 0.7616
            if new_plot.qm_dbh != 0:
                mean_dbh = parD0*(old_plot.age**parD1)*(old_plot.mean_h**parD2)
            else:
                mean_dbh = 0
            new_plot.set_value('MEAN_DBH', mean_dbh)  # store Average diameter in database
