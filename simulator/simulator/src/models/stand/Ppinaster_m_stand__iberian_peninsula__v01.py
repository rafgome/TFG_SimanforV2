...
# Pure stands of Pinus pinaster mesogeensis located on Spain
# Version 01
#
...
class PinusPinasterIberianStand(StandModel):

    def __init__(self, configuration=None):
        super().__init__(name="Pinus Pinaster Mesogeensis Iberian Peninsula Stand", version=1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Ppinaster_m_stand__iberian_peninsula__v01'  # set the model name to show it at the output
            ...
            Model.aplication_area = 'Península IBérica Mediterránea/Mediterranean Iberian Peninsula (Spain)'  # area recommended to use the model
            Model.valid_prov_reg = '2 - 20'  # provenance regions recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/masa/Ppinaster_mesogeensis_ip_stand_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/masa/Ppinaster_mesogeensis_ip_stand_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('       Running: Pinus pinaster mesogeensis stand model (Iberian Peninsula). Plot:', plot.plot_id     )
        print('#--------------------------------------------------------------------------------------------------#')

        try:

            self.model_info()
            plot.add_value('REINEKE_VALUE', -1.629)  # r constant value of SDI to the species of the model (-1.605 as default)   
...
#-------------------- Plot data calculation after having all the needed input variables -------------------------#
            ...
            if plot.si == 0 or plot.si == '':               
                t2 = 80  # SI reference age (years)
                plot.add_value('REF_SI_AGE', t2) 

                a = 4.016
                b = -0.5031
                SI = math.exp(a + (math.log(plot.dominant_h) - a)*((t2/plot.age)**b))
            ...
            b2 = -34.2235
            b3 = 0.849985
            V = math.exp(b0 + b1*plot.si + (b2/plot.age) + b3*math.log(plot.basal_area))
            ...
            if Area.september_temp[plot.plot_id] and Area.november_temp[plot.plot_id] and Area.september_rain[plot.plot_id] and Area.november_rain[plot.plot_id] != '':
    ...
    def growth(self, old_plot: Plot, new_plot: Plot, time: int):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('       Running: Pinus pinaster mesogeensis stand model (Iberian Peninsula). Plot:', old_plot.plot_id     )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            a = 4.016
            b = -0.5031
            Ho = math.exp(a + (math.log(old_plot.dominant_h) - a)*((new_age/old_plot.age)**b))
            new_plot.add_value('DOMINANT_H', Ho)  # store Dominant height in database

            # Basal Area     
            a0 = 4.746618
            a1 = 0  # that number is not at the document, but Miren del Río (responsible) confirmed that value
            G = math.exp((old_plot.age/new_age)*math.log(old_plot.basal_area) + ((a0 + a1*old_plot.si)*(1 - old_plot.age/new_age)))
            new_plot.add_value('BASAL_AREA', G)  # update plot basal area after the cut

            # Mortality
            d0 = -0.00069
            N = old_plot.density*math.exp(d0*(new_age - old_plot.age))
            ...
            b2 = -34.2235
            b3 = 0.849985
            V_dead = math.exp(b0 + b1*old_plot.si + (b2/old_plot.age) + b3*math.log(new_plot.dead_ba))
            ...
            b2 = -34.2235
            b3 = 0.849985
            V = math.exp(b0 + b1*old_plot.si + (b2/new_age) + b3*(old_plot.age/new_age)*math.log(new_plot.basal_area) + b3*(a0 + a1*old_plot.si)*(1 - old_plot.age/new_age))
            ...
            if Area.september_temp[new_plot.plot_id] and Area.november_temp[new_plot.plot_id] and Area.september_rain[new_plot.plot_id] and Area.november_rain[new_plot.plot_id] != '':
                self.mushrooms(new_plot)  # activate mushrooms function
    ...
    def harvest(self, old_plot: Plot, new_plot: Plot, cut_criteria, volume, time, min_age, max_age):
        ...
        Quadratic Mean Diameter after thinning equation:
            Doc.: Bravo-Oviedo A, Del Río M, Montero G (2004). Site index curves and growth model for Mediterranean maritime pine (Pinus pinaster Ait.) in Spain. Forest Ecology and Management, 201(2-3), 187-197
            Ref.: Bravo-Oviedo et al, 2004 
        ...
        try:
            ...
            # Volume Over Bark parameters
            b0 = 1.916498
            b1 = 0.061317
            b2 = -34.2235
            b3 = 0.849985

            # Quadratic Mean Diameter parameters
            c0 = 1.56059
            c1 = 0.97928
            c2 = 0.66444
            dgbt = old_plot.qm_dbh
            Gbt = old_plot.basal_area
            ...
            if cut_criteria == CUTS_DICT['PERCENTOFTREES']:
                ...
                if N != 0:
                    ...
                    dg  = c0 + c1*dgbt + c2*dgbt*((Ge/Gbt)**2)  # Quadratic Mean Diameter
                    V = math.exp(b0 + b1*old_plot.si + (b2/old_plot.age) + b3*math.log(G))
                else:
                    G = dg = V = 0
                ...
                print('')
                print('CUT CRITERIA NOT AVAILABLE BY THE MOMENT.')
                print('PLEASE, CHANGE THE CUT CRITERIA OF YOUR SCENARIO AND RESTART THE SIMULATION')
                print('')
...
# underestimated in the two first intervals (5 and 10
# years) whereas in long time projection, for 15 and 20
# years, the volume is overestimated, resulting in
# increasing mean absolute values.
    ...
    def mushrooms(self, plot):
        ...
        Mushrooms production on fresh weight (kg/ha):
            Doc.: Herrero C, Berraondo I, Bravo F, Pando V, Ordóñez C, Olaizola J, ... & Oria de Rueda J A (2019). Predicting mushroom productivity from long-term field-data series in Mediterranean Pinus pinaster Ait. forests in the context of climate change. Forests, 10(3), 206
            Ref.: Herrero et al, 2019
        """

        try:  # errors inside that construction will be announced

            #Y = total fresh-weight mushroom productivity (kg fw ha − 1 )
            #SMI = stand management indicator: G/N ((m2/ha)/(trees/ha))
            #TsPs = September average Temperature * precipitation (ºC and mm)
            #TnPn = November average Temperature * precipitation (ºC and mm)
            SMI = plot.basal_area/plot.density
            b0 = 10.5544
            b1 = 3.1786
            b2 = 0.000906
            b3 = 0.001926

            Ts = float(Area.september_temp[plot.plot_id])
            Ps = float(Area.september_rain[plot.plot_id])
            Tn = float(Area.november_temp[plot.plot_id])
            Pn = float(Area.november_rain[plot.plot_id])

            Y = math.exp(b0 + b1*math.log(SMI) + b2*Ts*Ps + b3*Tn*Pn) - 1

            if Y < 0:  # avoid negative values
                Y = 0

            plot.add_value('MUSHROOM_PRODUCTIVITY', Y)
...
PinusPinasterIberianStand.vars()