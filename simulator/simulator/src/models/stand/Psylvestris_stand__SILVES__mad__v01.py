...
# Pure stands of Pinus sylvestris located on Madrid (Spain)
# Version 01
#
...
class PinusSylvestrisMadridStand(StandModel):

    def __init__(self, configuration=None):
        super().__init__(name="Silves Madrid", version=1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Psylvestris_stand__SILVES__mad__v01'  # set the model name to show it at the output
            ...
            Model.aplication_area = 'Madrid & Guadarrama (Spain)'  # area recommended to use the model
            Model.valid_prov_reg = '10'  # provenance regions recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/masa/Psylvestris_stand_SILVES_mad_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/masa/Psylvestris_stand_SILVES_mad_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        Site Index equation:
            Doc.: del Río M, Roig S, Cañellas I, Montero G (2005). Programación de claras en repoblaciones de Pinus sylvestris L. Seguimiento de sitios de ensayo en la Comunidad de Madrid. Monografías del INIA: Serie forestal, 12, 46
            Ref.: del Río et al, 2005
        Mean Height:
            Doc.: del Río M, Roig S, Cañellas I, Montero G (2005). Programación de claras en repoblaciones de Pinus sylvestris L. Seguimiento de sitios de ensayo en la Comunidad de Madrid. Monografías del INIA: Serie forestal, 12, 46
            Ref.: del Río et al, 2005
        Mean Diameter:
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('              Running: Pinus sylvestris "SILVES" stand model (Madrid). Plot:', plot.plot_id          )
        print('#--------------------------------------------------------------------------------------------------#')
...
#-------------------- Plot data calculation after having all the needed input variables -------------------------#
            ...
            # Site Index
            t2 = 50  # reference age to SI calculation
            plot.add_value('REF_SI_AGE', t2)   

            if plot.si == 0 or plot.si == '':
                SI = plot.dominant_h*math.exp(-25.7121*(1/t2 - 1/plot.age))  # equation for Site Index
            ...
            # Mean Height 
            mean_h = -2.71929 + 0.84961*plot.dominant_h + 0.17122*plot.qm_dbh  # calculus of mean height
            plot.add_value('MEAN_H', mean_h)  # store mean height in database

            # Minimum Diameter
            min_dbh = -4.9124 + 0.6434*plot.qm_dbh
            plot.add_value('DBH_MIN', min_dbh)  # store minimum diameter in database

            # Mean Diameter
            mean_dbh = -0.6329 + 1.0058*plot.qm_dbh
            ...
            V = math.exp(1.3324 + 0.0559*plot.si - 22.7815/plot.age + 1.0394*math.log(plot.basal_area))  # equation for stand volumen
    ...
    def growth(self, old_plot: Plot, new_plot: Plot, time: int):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('              Running: Pinus sylvestris "SILVES" stand model (Madrid). Plot:', old_plot.plot_id          )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            # Dominant Height
            Ho = old_plot.dominant_h*math.exp(-25.7121*(1/new_age - 1/old_plot.age))  # Dominant height at plot age for actual site index 
            new_plot.add_value('DOMINANT_H', Ho)  # store Dominant height in database

            # Basal Area        
            G = (old_plot.basal_area**(old_plot.age/new_age)*math.exp(4.9272*(1 - old_plot.age/new_age)))  # basal area calculus
            new_plot.add_value('BASAL_AREA', G)  # update plot basal area after the cut

            # Mortality
            N = ((old_plot.density**(-2.34935)) + 0.99e-7*((new_age/100)**4.87390 - (old_plot.age/100)**4.87390))**(-1/2.34935)  # density calculus; new value of trees per hectare
            ...
            V_dead = math.exp(1.3324 + 0.0559*old_plot.si - 22.7815/old_plot.age + 1.0394*math.log(new_plot.dead_ba))  # equation for stand volumen
            new_plot.add_value('DEAD_VOL', V_dead)  # store dead volume in database

            # Volume Over Bark
            V = math.exp(1.3324 + 0.0559*old_plot.si - 22.7815/new_age + 1.0394*(old_plot.age/new_age)*math.log(old_plot.basal_area) + 1.0394*4.9272*(1 - old_plot.age/new_age))
            ...
            # Mean Height 
            mean_h = -2.71929 + 0.84961*new_plot.dominant_h + 0.17122*new_plot.qm_dbh  # calculus of mean height
            new_plot.add_value('MEAN_H', mean_h)  # store mean height in database

            # Minimum Diameter
            min_dbh = -4.9124 + 0.6434*new_plot.qm_dbh
            new_plot.add_value('DBH_MIN', min_dbh)  # store minimum diameter in database

            # Mean Diameter
            mean_dbh = -0.6329 + 1.0058*new_plot.qm_dbh
    ...
    def harvest(self, old_plot: Plot, new_plot: Plot, cut_criteria, volume, time, min_age, max_age):
        ...
        try:
            ...
            if cut_criteria == CUTS_DICT['PERCENTOFTREES']:
                ...
                if N != 0:
                    dg = 0.5310 + 0.9898*old_plot.qm_dbh + 0.5178*old_plot.qm_dbh*(tpuN**2)
                    tpuG = (math.sqrt(dg) - 0.1449 - 0.9698*math.sqrt(old_plot.qm_dbh))/0.6780  # G cut (%)
                    G = (1 - tpuG)*old_plot.basal_area
                    V = math.exp(1.3324 + 0.0559*old_plot.si - 22.7815/old_plot.age + 1.0394*math.log(G))  # equation to calculate volume
                else:
                    G = dg = V = 0
                ...
                if G != 0:
                    dg = (0.1449 + 0.9698*math.sqrt(old_plot.qm_dbh) + 0.6780*tpuBA)**2
                    tpuN = math.sqrt((dg - 0.5310 - 0.9898*old_plot.qm_dbh)/(0.5178*old_plot.qm_dbh))
                ...
                if V != 0:
                    G = math.exp((math.log(V) - 1.3324 - 0.0559*old_plot.si + 22.7815/old_plot.age)/1.0394)
            ...
            if new_plot.qm_dbh != 0:
                mean_h = -2.71929 + 0.84961*old_plot.dominant_h + 0.17122*new_plot.qm_dbh  # calculus of mean height
                # that 2 functions are defferent after a cut, so be carefull with the changes
                min_dbh = -7.0390 + 0.91684*new_plot.qm_dbh  
                mean_dbh = -0.4036 + 1.0041*new_plot.qm_dbh
            else:
...
PinusSylvestrisMadridStand.vars()