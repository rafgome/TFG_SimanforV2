...
# Pure stands of Pinus sylvestris located on Galicia (Spain)
# Version 01
#
...
class PinusSylvestrisGaliciaStand(StandModel):

    def __init__(self, configuration=None):
        super().__init__(name="Pinus Sylvestris Galicia Stand model", version=1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Psylvestris_stand__gal__v01'  # set the model name to show it at the output
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/masa/Psylvestris_stand_gal_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/masa/Psylvestris_stand_gal_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        Site Index equation:
            Doc.: Diéguez-Aranda U, Álvarez JG, Barrio-Anta M, Rojo A (2005). Site quality equations for Pinus sylvestris L. plantations in Galicia (northwestern Spain). Annals of Forest Science, 62(2), 143-152
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                  Running: Pinus sylvestris stand model (Galicia). Plot:', plot.plot_id              )
        print('#--------------------------------------------------------------------------------------------------#')
...
#-------------------- Plot data calculation after having all the needed input variables -------------------------#
            ...
            if plot.si == 0 or plot.si == '':
                t2 = 40  # SI reference age (years)
                SI = 51.39 / (1 - (1 - 51.39/plot.dominant_h)*((plot.age/t2)**1.277))
                plot.add_value('REF_SI_AGE', t2)   
                plot.add_value('SI', SI)  # store Site Index in database

            # Basal Area
            G = 92.40*math.exp( - (1593/plot.si)*plot.age**(-1.369))
            ...
            # Mean Diameter
            mean_dbh = plot.qm_dbh - math.exp(-1.294 + 0.0001867*plot.density + 0.03625*plot.dominant_h)
            plot.add_value('MEAN_DBH', mean_dbh)
         
            # Mean Height
            if plot.dominant_dbh != 0 and plot.mean_dbh != 0:
                mean_h = 1.3 + (plot.dominant_h - 1.3)*math.exp(7.197*(1/plot.dominant_dbh - 1/plot.mean_dbh))
            ...
            V = 0.5908*(plot.basal_area**0.9981)*(plot.dominant_h**0.8844)
    ...
    def growth(self, old_plot: Plot, new_plot: Plot, time: int):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                  Running: Pinus sylvestris stand model (Galicia). Plot:', old_plot.plot_id              )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            # Dominant Height
            Ho = 51.39/(1 - (1 - 51.39/old_plot.dominant_h)*((old_plot.age/new_age)**1.277))
            new_plot.add_value('DOMINANT_H', Ho)  # store Dominant height in database

            # Basal Area        
            G = 92.40*((old_plot.basal_area/92.40)**((old_plot.age/new_age)**1.369))
            new_plot.add_value('BASAL_AREA', G)  # update plot basal area after the cut

            # Mortality
            N = (old_plot.density**(-1.590) + 1.138e-12*old_plot.si*(new_age**3.308 - old_plot.age**3.308))**(-1/1.590)
            ...
            V_dead = 0.5908*(new_plot.dead_ba**0.9981)*(old_plot.dominant_h**0.8844)
            new_plot.add_value('DEAD_VOL', V_dead)  # store dead volume in database

            # Volume Over Bark
            V = 0.5908*(new_plot.basal_area**0.9981)*(new_plot.dominant_h**0.8844)
            ...
            # Mean Diameter
            mean_dbh = new_plot.qm_dbh - math.exp(-1.294 + 0.0001867*new_plot.density + 0.03625*new_plot.dominant_h)
            ...
            if new_plot.dominant_dbh != 0 and new_plot.mean_dbh != 0:
                mean_h = 1.3 + (new_plot.dominant_h - 1.3)*math.exp(7.197*(1/new_plot.dominant_dbh - 1/new_plot.mean_dbh))
        ...
        try:
            ...
            if cut_criteria == CUTS_DICT['PERCENTOFTREES']:
                ...
                if N != 0:
                    ...
                    dg  = 2*math.sqrt(MTBA/math.pi)  # Quadratic Mean Diameter
                    V = 0.5908*(G**0.9981)*(old_plot.dominant_h**0.8844)  # equation to calculate volume
                else:
                    G = dg = V = 0
                ...
                if V != 0:
                    G = (V/(0.5908*(old_plot.dominant_h**0.8844)))**(1/0.9981)
                    N = old_plot.density*G/old_plot.basal_area
                    MTBA = G*10000/N  # Mean Tree Basal Area
            ...
            new_plot.add_value('DEAD_VOL', 0)  # Dead volume over bark           
...
PinusSylvestrisGaliciaStand.vars()