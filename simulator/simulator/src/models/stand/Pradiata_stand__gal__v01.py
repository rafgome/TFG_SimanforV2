...
# Pure stands of Pinus radiata located on Galicia (Spain)
# Version 01
#
...
class PinusRadiataGaliciaStand(StandModel):

    def __init__(self, configuration=None):
        super().__init__(name="Pinus Radiata Galicia model", version=1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Pradiata_stand__gal__v01'  # set the model name to show it at the output
            Model.specie_ifn_id = 28  # Set the model specie ID to mark the trees of different species               
            ...
            Model.valid_prov_reg = '1, 2, 3, 4, 5'  # provenance regions recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/masa/Pradiata_gal_stand_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/masa/Pradiata_gal_stand_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        Site Index equation:
            Doc.: Diéguez-Aranda U, Burkhart H, Rodríguez-Soalleiro R (2005). Modeling dominant height growth of radiata pine (Pinus radiata D. Don) plantations in north-western Spain. Forest Ecology and Management, 215(1-3), 271-284
            Ref.: Diéguez-Aranda et al, 2005
        Basal Area equation:
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                   Running: Pinus radiata stand model (Galicia). Plot:', plot.plot_id                )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            if Area.specie_ifn_id[plot.plot_id] != Model.specie_ifn_id:
                print()
                print('BE CAREFUL! You are trying to run that model of Pinus radiata with an inventory of another specie.')
...
#-------------------- Plot data calculation after having all the needed input variables -------------------------#
            ...
            if plot.si == 0 or plot.si == '':
                ...
                SI = plot.dominant_h*(((1 - math.exp(-0.06738*t2)) / (1 - math.exp(-0.06738*plot.age)))**(-1.755 + 12.44/Xo))
                plot.add_value('REF_SI_AGE', t2)   
                plot.add_value('SI', SI)  # store Site Index in database

            # Basal Area
            G = -52.23 + 2.676*plot.age + 1.306*plot.si + 0.01008*plot.density
            ...
            # Mean Diameter
            mean_dbh = plot.qm_dbh - math.exp(0.1449 - 19.76/plot.age + 0.0001345*plot.density + 0.03264*plot.si)
            plot.add_value('MEAN_DBH', mean_dbh)

            # Mean Height and Slenderness
            if plot.dominant_dbh != 0 and plot.dominant_dbh != "":
                mean_h = (1.3**1.633 + (plot.dominant_h**1.633 - 1.3**1.633)*((1 - math.exp(-0.02961*plot.mean_dbh))/(1 - math.exp(-0.02961*plot.dominant_dbh))))**(1/1.633)
            ...
            V = (plot.basal_area*plot.dominant_h)**(0.9987-0.002232*plot.age)*math.exp(-0.9635 + 0.01703*plot.age)
            plot.add_value('VOL', V)  # store initial stand volume in database

            # Biomass
            WT = 2.319e-5*(plot.qm_dbh**1.928)*(plot.dominant_h**0.9210)*(plot.density**1.007)
    ...
    def growth(self, old_plot: Plot, new_plot: Plot, time: int):
        ...
        Reineke Index equation: standad value
        Hart Index equation --> standard equation
        """

        print('#------------------------------------------------------------------------------------------------- #')
        print('                   Running: Pinus radiata stand model (Galicia). Plot:', old_plot.plot_id                )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            # Dominant Height
            Lo = math.log(1 - math.exp(-0.06738*old_plot.age))
            Xo = 0.5*((math.log(old_plot.dominant_h) + 1.755*Lo) + math.sqrt((math.log(old_plot.dominant_h) + 1.755*Lo)**2 - 4*12.44*Lo))
            Ho = old_plot.dominant_h*(((1 - math.exp(-0.06738*new_age)) / (1 - math.exp(-0.06738*old_plot.age)))**(-1.755+12.44/Xo))
            new_plot.add_value('DOMINANT_H', Ho)  # store Dominant height in database

            # Basal Area        
            Xo = (old_plot.age**(-0.9233)/2)*(-276.1 + old_plot.age**(0.9233)*math.log(old_plot.basal_area) + math.sqrt(4*1391*old_plot.age**(0.9233) + (276.1 - old_plot.age**(0.9233)*math.log(old_plot.basal_area))**2))
            G = math.exp(Xo)*math.exp(-(-276.1 + 1391/Xo)*(new_age**(-0.9233)))
            new_plot.add_value('BASAL_AREA', G)  # update plot basal area after the cut

            # Mortality
            N = (old_plot.density**(-0.3161) + 1.053**(new_age - 100) - 1.053**(old_plot.age - 100))**(-1/0.3161)
            ...
            V_dead = (new_plot.dead_ba*old_plot.dominant_h)**(0.9987-0.002232*old_plot.age)*math.exp(-0.9635 + 0.01703*old_plot.age)
            new_plot.add_value('DEAD_VOL', V_dead)  # store dead volume in database   

            # Volume Over Bark
            V = (new_plot.basal_area*new_plot.dominant_h)**(0.9987-0.002232*new_age)*math.exp(-0.9635 + 0.01703*new_age)
            ...
            # Mean Diameter
            mean_dbh = new_plot.qm_dbh - math.exp(0.1449 - 19.76/new_age + 0.0001345*new_plot.density + 0.03264*old_plot.si)
            ...
            if new_plot.dominant_dbh != 0 and new_plot.dominant_dbh != "":
                mean_h = (1.3**1.633 + (new_plot.dominant_h**1.633 - 1.3**1.633)*((1 - math.exp(-0.02961*new_plot.mean_dbh))/(1 - math.exp(-0.02961*new_plot.dominant_dbh))))**(1/1.633)
            ...
            # Biomass
            WT = 2.319e-5*(new_plot.qm_dbh**1.928)*(new_plot.dominant_h**0.9210)*(new_plot.density**1.007)
    ...
    def harvest(self, old_plot: Plot, new_plot: Plot, cut_criteria, volume, time, min_age, max_age):
        ...
        try:
            ...
            if cut_criteria == CUTS_DICT['PERCENTOFTREES']:
                ...
                if N != 0:
                    G = -52.23 + 2.676*old_plot.age + 1.306*old_plot.si + 0.01008*N
                    MTBA = G*10000/N  # Mean Tree Basal Area
                    dg  = 2*math.sqrt(MTBA/math.pi)  # Quadratic Mean Diameter
                    V = (G*old_plot.dominant_h)**(0.9987-0.002232*old_plot.age)*math.exp(-0.9635 + 0.01703*old_plot.age)  # equation to calculate volume
                else:
                    G = dg = V = 0
                ...
                if G != 0:
                    N = (G + 52.23 - 2.676*old_plot.age - 1.306*old_plot.si)/0.01008
                    MTBA = G*10000/N  # Mean Tree Basal Area
                    dg  = 2*math.sqrt(MTBA/math.pi)  # Quadratic Mean Diameter
                    V = (G*old_plot.dominant_h)**(0.9987-0.002232*old_plot.age)*math.exp(-0.9635 + 0.01703*old_plot.age)  # equation to calculate volume
                else:
                ...
                if V != 0:
                    G = math.exp((math.log(V/math.exp(-0.9635 + 0.01703*old_plot.age)))/(0.9987-0.002232*old_plot.age))/old_plot.dominant_h
                    N = (G + 52.23 - 2.676*old_plot.age - 1.306*old_plot.si)/0.01008
                    MTBA = G*10000/N  # Mean Tree Basal Area
            ...
            # Mean Diameter
            if new_plot.qm_dbh != 0:
                mean_dbh = new_plot.qm_dbh - math.exp(0.1449 - 19.76/old_plot.age + 0.0001345*new_plot.density + 0.03264*old_plot.si)
            else:
                mean_dbh = 0
            new_plot.add_value('MEAN_DBH', mean_dbh)
