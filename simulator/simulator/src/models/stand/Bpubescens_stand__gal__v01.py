...
# Pure stands of Betula pubescens located on Galicia (Spain)
# Version 01
#
...
class BetulaPubescensGaliciaStand(StandModel):

    def __init__(self, configuration=None):
        super().__init__(name="Betula pubescens stand model", version=1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Bpubescens_stand__gal__v01'  # set the model name to show it at the output
            Model.specie_ifn_id = 73  # Set the model specie ID to mark the trees of different species         
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/masa/Bpubescens_gal_stand_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/masa/Bpubescens_gal_stand_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        Site Index equation:
            Doc.: Gómez-García E, Crecente-Campo F, Stankova T, Rojo A, Diéguez-Aranda U (2010). Dynamic growth model for Birch stands in northwestern Spain. FORESTRY, 16(2), 40
            Ref.: Gómez-García et al, 2010
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                 Running: Betula pubescens stand model (Galicia). Plot:', plot.plot_id               )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            if Area.specie_ifn_id[plot.plot_id] != Model.specie_ifn_id:
                print()
                print('BE CAREFUL! You are trying to run that model of Betula pubescens with an inventory of another specie.')
...
#-------------------- Plot data calculation after having all the needed input variables -------------------------#
            ...
            if plot.si == 0 or plot.si == '':            
                ...
                Xo = (plot.dominant_h - 19.80 + math.sqrt((19.80 - plot.dominant_h)**2 + 3032*plot.dominant_h*(plot.age**-1.398)))/2
                SI = (19.80 + Xo)/(1 + (758/Xo)*(t2**(-1.398))) 
                plot.add_value('SI', SI)  # store Site Index in database

            # Quadratic Mean Diameter
            dg = 13.86*(plot.density**-0.2721)*(plot.dominant_h**0.74)
            plot.add_value('QM_DBH', dg)

            # Basal Area
            G = -77.14 + 0.1189*plot.age + 1.894*plot.dominant_h + 9.559*math.log(plot.density)
            plot.add_value('BASAL_AREA', G)

            # Mean Diameter
            mean_dbh = plot.qm_dbh - math.exp(-1.418 + 0.0884*plot.dominant_h - 0.0395*plot.si)
            plot.add_value('MEAN_DBH', mean_dbh) 

            # Mean Height and Slenderness
            if plot.mean_dbh != 0 and plot.dominant_dbh != 0:
                mean_h = 1.3 + (plot.dominant_h - 1.3)*math.exp((-0.3834*plot.dominant_h + 0.5139*plot.density/1000)*(1/plot.mean_dbh - 1/plot.dominant_dbh)) 
            ...
            V = 3.898e-5*(plot.qm_dbh**2.219)*(plot.dominant_h**0.6523)*(plot.density**1.015)
    ...
    def growth(self, old_plot: Plot, new_plot: Plot, time: int):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print('                 Running: Betula pubescens stand model (Galicia). Plot:', old_plot.plot_id               )
        print('#--------------------------------------------------------------------------------------------------#')
        ...
        try:
            ...
            # Dominant Height
            Xo = 0.5*(old_plot.dominant_h - 19.80 + math.sqrt((19.80 - old_plot.dominant_h)**2 + 3032*old_plot.dominant_h*(old_plot.age**(-1.398))))
            Ho = (19.80 + Xo)/(1 + (758/Xo)*(new_age**(-1.398))) 
            new_plot.add_value('DOMINANT_H', Ho)  # store Dominant height in database

            # Mortality
            N = (old_plot.density**(-1/1.577) + 0.01255*((new_age/100)**1.577 - (old_plot.age/100)**1.577))**-1.577
            ...
            V_dead = 3.898e-5*(old_plot.qm_dbh**2.219)*(old_plot.dominant_h**0.6523)*(new_plot.dead_density**1.015)            
            new_plot.add_value('DEAD_VOL', V_dead)  # store dead volume in database            
            
            # Quadratic Mean Diameter
            dg = 13.86*(new_plot.density**-0.2721)*(new_plot.dominant_h**0.74)
            new_plot.add_value('QM_DBH', dg)

            # Basal Area        
            G = old_plot.basal_area**(old_plot.age/new_age)*math.exp(2.290*(1 - old_plot.age/new_age) + 0.2521*(math.log(new_plot.density) - (old_plot.age/new_age)*math.log(old_plot.density)))
            new_plot.add_value('BASAL_AREA', G)  # update plot basal area after the cut

            # Volume Over Bark
            V = 3.898e-5*(new_plot.qm_dbh**2.219)*(new_plot.dominant_h**0.6523)*(new_plot.density**1.015)
            new_plot.add_value('VOL', V)  # update plot volume over bark after the cut

            # Mean Diameter
            mean_dbh = new_plot.qm_dbh - math.exp(-1.418 + 0.0884*new_plot.dominant_h - 0.0395*old_plot.si)
            ...
            if new_plot.mean_dbh != 0 and new_plot.dominant_dbh != 0:
                mean_h = 1.3 + (new_plot.dominant_h - 1.3)*math.exp((-0.3834*new_plot.dominant_h + 0.5139*new_plot.density/1000)*(1/new_plot.mean_dbh - 1/new_plot.dominant_dbh))
    ...
    def harvest(self, old_plot: Plot, new_plot: Plot, cut_criteria, volume, time, min_age, max_age):
        ...
        try:
            ...
            if cut_criteria == CUTS_DICT['PERCENTOFTREES']:
                ...
                if N != 0:
                    G = -77.14 + 0.1189*old_plot.age + 1.894*old_plot.dominant_h + 9.559*math.log(N)  # Basal area
                    dg  = 13.86*(N**-0.2721)*(old_plot.dominant_h**0.74)  # Quadratic Mean Diameter
                    V = 3.898e-5*(dg**2.219)*(old_plot.dominant_h**0.6523)*(N**1.015)  # Volume Over Bark
                else:
                    G = dg = V = 0
                ...
                if G != 0:
                    N = math.exp((G + 77.14 - 0.1189*old_plot.age - 1.894*old_plot.dominant_h)/9.559)  # Density
                    dg  = 13.86*(N**-0.2721)*(old_plot.dominant_h**0.74)  # Quadratic Mean Diameter
                    V = 3.898e-5*(dg**2.219)*(old_plot.dominant_h**0.6523)*(N**1.015)  # Volume Over Bark
                else:
                    N = dg = V = 0
                ...
                if V != 0:
                    N = (V/(0.0139*(old_plot.dominant_h**2.2944)))**2.4319  # Density
                    dg  = 13.86*(N**-0.2721)*(old_plot.dominant_h**0.74)  # Quadratic Mean Diameter
                    G = -77.14 + 0.1189*old_plot.age + 1.894*old_plot.dominant_h + 9.559*math.log(N)  # Basal area
                else:
                    G = dg = N = 0
            ...
            new_plot.add_value('DEAD_VOL', 0)  # Dead volume
            ...
            if new_plot.mean_dbh != 0 and new_plot.dominant_dbh != 0:
                ...
                mean_h = slenderness_dom = slenderness_mean = 0
            new_plot.add_value('MEAN_H', mean_h)  # store mean height in database
...
BetulaPubescensGaliciaStand.vars()