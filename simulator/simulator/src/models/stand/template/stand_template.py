...
#-------------------- Plot data calculation after having all the needed input variables -------------------------#
            ...
            # activate them just if they are calculated on the model
    ...
    def growth(self, old_plot: Plot, new_plot: Plot, time: int):
        ...
        try:
            ...
            new_plot.add_value('DEAD_VOL', old_plot.vol*new_plot.dead_density/old_plot.density)

            # Biomass
            WT =
            new_plot.add_value('WT', WT)
            new_plot.add_value('DEAD_WT', old_plot.wt*new_plot.dead_density/old_plot.density)
