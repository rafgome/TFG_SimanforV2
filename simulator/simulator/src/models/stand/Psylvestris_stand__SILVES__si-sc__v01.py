...
# Pure stands of Pinus sylvestris located on Sistema Ibérico and Sistema Central (Spain)
# Version 01
#
...
class PinusSylvestrisSISCStand(StandModel):

    def __init__(self, configuration=None):
        super().__init__(name="Silves Iberian and Central System", version=1)
    ...
    def model_info(self):
        ...
        try:  # errors inside that construction will be announced
            ...
            Model.model_name = 'Psylvestris_stand__SILVES__si-sc__v01'  # set the model name to show it at the output
            Model.specie_ifn_id = 21  # Set the model specie ID to mark the trees of different species               
            Model.exec_time = '10 - 15'  # recommended executions time to use that model
            Model.aplication_area = 'Sistema Ibérico Central/Iberian Central System (Spain)'  # area recommended to use the model
            Model.valid_prov_reg = '8, 10'  # provenance regions recommended to use the model
            ...
            Model.model_card_en = 'https://github.com/simanfor/modelos/blob/main/masa/Psylvestris_stand_SILVES_sisc_EN.pdf'  # link to model card in english
            Model.model_card_es = 'https://github.com/simanfor/modelos/blob/main/masa/Psylvestris_stand_SILVES_sisc_ES.pdf'  # link to model card in spanish
    ...
    def initialize(self, plot: Plot):
        ...
        Mean Height:
            Doc.: del Río M, Montero G (2001). Modelo de simulación de claras en masas de Pinus sylvestris L (No. 04; SD396. 5, R5.). INIA
            Ref.: del Río and Montero, 2001  
        Mean Diameter:
            Doc.: del Río M, Montero G (2001). Modelo de simulación de claras en masas de Pinus sylvestris L (No. 04; SD396. 5, R5.). INIA
            Ref.: del Río and Montero, 2001  
        Minimum Diameter:
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print(' Running: Pinus sylvestris "SILVES" stand model (Iberian and Central Systems). Plot:', plot.plot_id  )
        print('#--------------------------------------------------------------------------------------------------#')
...
#-------------------- Plot data calculation after having all the needed input variables -------------------------#
            ...
            # Site Index
            t2 = 100  # reference age to SI calculation
            plot.add_value('REF_SI_AGE', t2)   

            if plot.si == 0 or plot.si == '':

                # Quality Index
                IC = ((plot.dominant_h/10)*0.8534446)/((1 - math.exp(-0.270*t2/10))**(1/0.439))

                # Site Index
                SI = IC*10  # equation for Site Index
            ...
            # Mean Height 
            mean_h = -1.155649 + 0.976772*plot.dominant_h  # calculus of mean height
            plot.add_value('MEAN_H', mean_h)  # store mean height in database

            # Minimum Diameter
            min_dbh = 2.891926 + 0.019246*plot.basal_area + 0.008571*(plot.qm_dbh**2)
            plot.add_value('DBH_MIN', min_dbh)  # store minimum diameter in database

            # Mean Diameter
            mean_dbh = 0.917473 + 0.007942*plot.basal_area + 0.954773*plot.qm_dbh - 1.464034*plot.dominant_h/plot.qm_dbh
            plot.add_value('MEAN_DBH', mean_dbh)  # store mean diameter in database

            # Slenderness
            plot.add_value('SLENDERNESS_MEAN', plot.mean_h*100/plot.mean_dbh)  

            # Initial Volume Over Bark
            V = math.exp(1.427060 + 0.388317*(plot.si/10) - 30.691629/plot.age + 1.034549*math.log(plot.basal_area))  # equation for stand volumen
    ...
    def growth(self, old_plot: Plot, new_plot: Plot, time: int):
        ...
        print('#------------------------------------------------------------------------------------------------- #')
        print(' Running: Pinus sylvestris "SILVES" stand model (Iberian and Central Systems). Plot:', old_plot.plot_id  )
        print('#--------------------------------------------------------------------------------------------------#')

        if time not in [10,15]:  # if the user time is not the same as the execution model time, a warning message will be notified
        ...
        try:
            ...
            # Dominant Height
            Ho = 10*(((old_plot.si/10)*(1 - math.exp(-0.270*(new_age/10))))/0.8534446)  # Dominant height at plot age for actual site index 
            new_plot.add_value('DOMINANT_H', Ho)  # store Dominant height in database

            # Basal Area        
            G = (old_plot.basal_area**(old_plot.age/new_age)*math.exp(5.103222*(1 - old_plot.age/new_age)))  # basal area calculus
            new_plot.add_value('BASAL_AREA', G)  # update plot basal area after the cut

            # Mortality
            N = ((old_plot.density**(-2.349350607)) + 0.99e-7*((new_age/100)**4.873897659 - (old_plot.age/100)**4.873897659))**(-1/2.349350607)  # density calculus; new value of trees per hectare
            ...
            V_dead = math.exp(1.427060 + 0.388317*(old_plot.si/10) - 30.691629/old_plot.age + 1.034549*math.log(new_plot.dead_ba)) 
            new_plot.add_value('DEAD_VOL', V_dead)  # store dead volume in database

            # Volume Over Bark
            V = math.exp(1.427060 + 0.388317*(old_plot.si/10) - 30.691629/new_age + 1.034549*(old_plot.age/new_age)*math.log(old_plot.basal_area) + 1.034549*5.103222*(1 - old_plot.age/new_age))
            ...
            # Mean Height 
            mean_h = -1.155649 + 0.976772*new_plot.dominant_h  # calculus of mean height
            new_plot.add_value('MEAN_H', mean_h)  # store mean height in database

            # Minimum Diameter
            min_dbh = 2.891926 + 0.019246*new_plot.basal_area + 0.008571*(new_plot.qm_dbh**2)
            new_plot.add_value('DBH_MIN', min_dbh)  # store minimum diameter in database

            # Mean Diameter
            mean_dbh = 0.917473 + 0.007942*new_plot.basal_area + 0.954773*new_plot.qm_dbh - 1.464034*new_plot.dominant_h/new_plot.qm_dbh
            new_plot.add_value('MEAN_DBH', mean_dbh)  # store mean diameter in database
    ...
    def harvest(self, old_plot: Plot, new_plot: Plot, cut_criteria, volume, time, min_age, max_age):
        ...
        try:
            ...
            if cut_criteria == CUTS_DICT['PERCENTOFTREES']:
                ...
                if N != 0:
                    dg = 0.531019 + 0.989792*old_plot.qm_dbh + 0.517850*old_plot.qm_dbh*(tpuN**2)
                    tpuG = (math.sqrt(dg) - 0.1449 - 0.9698*math.sqrt(old_plot.qm_dbh))/0.6780  # G cut (%)
                    G = (1 - tpuG)*old_plot.basal_area
                    V = math.exp(1.427060 + 0.388317*(old_plot.si/10) - 30.691629/old_plot.age + 1.034549*math.log(G))  # equation to calculate volume
                else:
                    G = dg = V = 0
                ...
                if G != 0:
                    dg = (0.144915 + 0.969819*math.sqrt(old_plot.qm_dbh) + 0.678010*tpuBA)**2
                    tpuN = math.sqrt((dg - 0.531019 - 0.989792*old_plot.qm_dbh)/(0.517850*old_plot.qm_dbh))
                    N = (1 - tpuN)*old_plot.density
                    V = math.exp(1.427060 + 0.388317*(old_plot.si/10) - 30.691629/old_plot.age + 1.034549*math.log(G))  # equation to calculate volume
                else:
                    N = dg = V = 0
                ...
                if V != 0:
                    G = math.exp((math.log(V) - 1.427060 - 0.388317*(old_plot.si/10) + 30.691629/old_plot.age)/1.034549)
                    tpuBA = (old_plot.basal_area - G)/old_plot.basal_area
                    dg = (0.144915 + 0.969819*math.sqrt(old_plot.qm_dbh) + 0.678010*tpuBA)**2
                    tpuN = math.sqrt((dg - 0.531019 - 0.989792*old_plot.qm_dbh)/(0.517850*old_plot.qm_dbh))
                    N = (1 - tpuN)*old_plot.density
                else:
            ...
            # Mean Height, Minimum Diameter and Mean Diameter
            #if new_plot.qm_dbh != 0:
                # mean h equation is the same as in executions, and the variables it uses doesn't change with the cut, so it has no sense to calculate it again after a cut, the value will be the same
                #mean_h = -1.155649 + 0.976772*old_plot.dominant_h  # calculus of mean height
                # that 2 functions are defferent after a cut, so be carefull with the changes
                # nc is the time between the last cut and the present one. By the moment we can't calculate it, so I silence the next equations
                #min_dbh = -5.554181 + 0.568817*new_plot.qm_dbh + 0.112089*nc + 11.213626*old_plot.dominant_h/old_plot.age
                #mean_dbh = -0.430105 + 0.997758*new_plot.qm_dbh - 0.007959*new_plot.basal_area + 0.006067*nc + 1.022125*old_plot.dominant_h/old_plot.age
            #else:
            ...
            # as mean dbh and mean h doesn't change, slenderness doesn't change too
            # Slenderness
            #if new_plot.qm_dbh != 0:
