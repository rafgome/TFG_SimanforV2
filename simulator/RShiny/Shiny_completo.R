...
# setwd("/home/rstudio/simulator/GraficosR/AbrirDatos")
setwd("/home/rstudio/data")


listaZip<-dir(pattern = "*.zip")

## Descomprimimos los zips en su carpeta correspondiente
listaEscenarios<-c()
# Comentar estas 4 líneas si no funciona
...
# Descomentar estas 2 líneas si no funciona
...
# Comentar o quitar este for si no funciona
for(i in 1:length(listaEscenarios)){
  ...
  for(i in 1:length(archivos)){
    plots <- tryCatch({
      read_excel(paste0(dirEscenario, "/", archivos[i]), sheet = "Plots")
    }, error=function(e){
      read_excel(paste0(dirEscenario, "/", archivos[i]), sheet = "Parcelas")
    })
    ...
    plotsArchivo <- tryCatch({
      read.xlsx(paste0(dirEscenario,"/",archivos[i]),startRow = 7,sheet="Summary")
    }, error = function(e) {
      read.xlsx(paste0(dirEscenario,"/",archivos[i]),startRow = 7,sheet="Resumen")
    })
...
# Descomentar las siguientes 5 líneas si no funciona
## for(i in 1:length(listaEscenarios)){
# idEscenario <- idEscenario+1
...
# Descomentar estas 5 líneas si no funciona
# plots <- tryCatch({
...
# Descomentar estas 6 líneas si no funciona
# # spiros: here need to remove the unnecessary variables
# lengthPlotsArchivo <- length(names(plotsArchivo))
# 
# sub <- lengthPlotsArchivo-length(plotsArchivoVars)
...
names(archivos_SBT_N)<-c("Age","Ho","SBT_N","IdZip", "IdNombre","IdArchivo","IdEscenario")
#Añadimos las variables que no estan para asi poderlo unir en un data frame por filas
archivos_SBT_N$listaZip<-datosArchivo$idZip
...
archivos_SBT_dg<-data.frame(cbind(datosArchivo$Age,datosArchivo$Ho,datosArchivo$SAT_dg,datosArchivo$IdZip,datosArchivo$IdNombre,datosArchivo$IdArchivo,datosArchivo$IdEscenario))
archivos_SBT_dg<-na.omit(archivos_SBT_dg)
names(archivos_SBT_dg)<-c("Age","Ho","SBT_dg","IdZip","IdNombre","IdArchivo","IdEscenario")
#Añadimos las variables que no estan para asi poderlo unir en un data frame por filas
...
archivos_SBT_G<-data.frame(cbind(datosArchivo$Age,datosArchivo$Ho,datosArchivo$SAT_G,datosArchivo$IdZip,datosArchivo$IdNombre,datosArchivo$IdArchivo,datosArchivo$IdEscenario))
archivos_SBT_G<-na.omit(archivos_SBT_G)
names(archivos_SBT_G)<-c("Age","Ho","SBT_G","IdZip","IdNombre","IdArchivo","IdEscenario")
#Añadimos las variables que no estan para asi poderlo unir en un data frame por filas
...
archivos_SBT_V<-data.frame(cbind(datosArchivo$Age,datosArchivo$Ho,datosArchivo$SAT_V,datosArchivo$IdZip,datosArchivo$IdNombre,datosArchivo$IdArchivo,datosArchivo$IdEscenario))
archivos_SBT_V<-na.omit(archivos_SBT_V)
names(archivos_SBT_V)<-c("Age","Ho","SBT_V","IdZip","IdNombre","IdArchivo","IdEscenario")
#Añadimos las variables que no estan para asi poderlo unir en un data frame por filas
...
# get scenario code
if("Scenario_file_name" %in% colnames(df_acum)){
...
if("Scenario_file_name" %in% colnames(df_acum)){
  ...
  df_medias_evolucion<- ddply(df_acum, c('IdZip', 'IdNombre', 'IdEscenario', 'T'), summarise, 
...
nombreZips<-unique(dfArchivo$IdZip)
escenarios<-unique(nombreZips)
...
server <- function(input, output,session) {
  ...
  output$graficoBiomasa <- renderPlotly({
    ...
    gb<-ggplot(dfFiltered, aes(x = T, y = eval(parse(text = input$variableB)), color = as.character(IdNombre), group = IdNombre)) +
      geom_point() +
      geom_line() +
      labs(title = bquote("Biomass vs Age"),
           ...
           color = "Scenario") +
      scale_color_discrete(labels = escenarios) +  # Asignar nombres de escenarios como etiquetas de la leyenda
      theme_bw()+
  ...
  output$graficoBiomasaAll <- renderPlotly({
    ...
    gb<-ggplot(dfFiltered, aes(x = T, y = eval(parse(text = input$variableB_all)), color = as.character(IdNombre), group = IdNombre)) +
      geom_point() +
      geom_line() +
  ...
  output$graficoVolumen <- renderPlotly({
    ...
    gv<-ggplot(dfFiltered, aes(x = T, y = eval(parse(text = input$variableV)), color = as.character(IdNombre), group = IdNombre)) +
      geom_point() +
      geom_line() +
  ...
  output$graficoVolumenAll <- renderPlotly({
    ...
    gv<-ggplot(dfFiltered, aes(x = T, y = eval(parse(text = input$variableV_all)), color = as.character(IdNombre), group = IdNombre)) +
      geom_point() +
      geom_line() +
